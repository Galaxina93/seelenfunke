...
<div x-data="{ mobileMenuOpen: false }" @click.outside="if(window.innerWidth < 768) mobileMenuOpen = false" class="pointer-events-auto relative">
    ...
    <div :class="mobileMenuOpen ? 'flex flex-col absolute right-0 top-12 mt-2 w-48 bg-gray-900/95 border border-gray-700 rounded-xl shadow-2xl backdrop-blur-xl p-2 z-[100]' : 'hidden md:flex md:flex-row md:items-center md:gap-2 md:static md:bg-transparent md:border-0 md:p-0 md:shadow-none'"
         class="gap-2 transition-transform">

         <select x-model="activeAgentId" @change="playAudio('click'); $wire.set('agentId', activeAgentId)" class="px-2 py-1.5 border rounded-lg text-[10px] font-black uppercase tracking-widest transition-all backdrop-blur-md bg-black text-white border-gray-700 hover:text-white focus:outline-none focus:border-emerald-500 appearance-none shadow-[0_0_15px_rgba(0,0,0,0.5)] w-full md:w-auto mb-2 md:mb-0">
         ...
         <select x-model="currentChatSessionId" @change="playAudio('click'); $wire.set('currentChatSessionId', currentChatSessionId)" class="px-2 py-1.5 border rounded-lg text-[10px] font-black uppercase tracking-widest transition-all backdrop-blur-md bg-black text-white border-gray-700 hover:text-white focus:outline-none focus:border-[var(--theme-color)] appearance-none shadow-[0_0_15px_rgba(0,0,0,0.5)] w-full md:w-auto mb-2 md:mb-0 max-w-[150px]">
         ...
         <select x-show="isMapFocus" x-model="activeMapStyle" @change="playAudio('click');" class="px-2 py-1.5 border rounded-lg text-[10px] font-black uppercase tracking-widest transition-all backdrop-blur-md bg-black text-white border-emerald-900/50 hover:text-white focus:outline-none focus:border-[var(--theme-color)] appearance-none shadow-[0_0_15px_rgba(0,0,0,0.5)] w-full md:w-auto mb-2 md:mb-0 max-w-[120px]">
             <template x-for="style in mapStyles" :key="style.id">
                 <option :value="style.id" x-text="style.name" class="bg-black text-white"></option>
             </template>
         </select>

         <button @click="playAudio('click'); toggleLiveMode(); if(window.innerWidth < 768) mobileMenuOpen = false;" class="px-3 py-2 border rounded-lg text-[10px] font-black uppercase tracking-widest transition-all backdrop-blur-md flex items-center gap-2 w-full md:w-auto justify-start md:justify-center"
         ...
         <button x-show="isMapFocus" x-transition @click="playAudio(isFlightDataActive ? 'unclick' : 'click'); isFlightDataActive = !isFlightDataActive; if(window.innerWidth < 768) mobileMenuOpen = false;" class="px-3 py-2 border rounded-lg text-[10px] font-black uppercase tracking-widest transition-all backdrop-blur-md flex items-center gap-2 w-full md:w-auto justify-start md:justify-center"
         ...
         <button @click="playAudio(isBrainFocus ? 'unclick' : 'brain-open'); $dispatch('ai-toggle-brain-workspace', {open: !isBrainFocus, silent: true}); if(window.innerWidth < 768) mobileMenuOpen = false;" class="px-3 py-2 border rounded-lg text-[10px] font-black uppercase tracking-widest transition-all backdrop-blur-md flex items-center gap-2 w-full md:w-auto justify-start md:justify-center"
             :class="isBrainFocus ? 'text-indigo-400 border-indigo-500/50 bg-indigo-900/30 hover:bg-indigo-800 shadow-[0_0_15px_rgba(99,102,241,0.5)] animate-pulse' : 'text-gray-500 border-gray-700 bg-gray-900/50 hover:text-gray-300 hover:bg-gray-800'">
             <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-4 h-4"><path d="M12 2.25a.75.75 0 0 1 .75.75v2.25a.75.75 0 0 1-1.5 0V3a.75.75 0 0 1 .75-.75ZM7.5 12a4.5 4.5 0 1 1 9 0 4.5 4.5 0 0 1-9 0ZM18.894 6.166a.75.75 0 0 0-1.06-1.06l-1.591 1.59a.75.75 0 1 0 1.06 1.061l1.591-1.59ZM21.75 12a.75.75 0 0 1-.75.75h-2.25a.75.75 0 0 1 0-1.5H21a.75.75 0 0 1 .75.75ZM17.834 18.894a.75.75 0 0 0 1.06-1.06l-1.59-1.591a.75.75 0 1 0-1.061 1.06l1.59 1.591ZM12 18.75a.75.75 0 0 1 .75.75V21a.75.75 0 0 1-1.5 0v-1.5a.75.75 0 0 1 .75-.75ZM6.166 18.894a.75.75 0 0 0 1.06 1.06l1.59-1.591a.75.75 0 1 0-1.06-1.061l-1.59 1.591ZM2.25 12a.75.75 0 0 1 .75-.75h2.25a.75.75 0 0 1 0 1.5H3a.75.75 0 0 1-.75-.75ZM6.166 5.106a.75.75 0 0 0-1.06 1.06l1.591 1.59a.75.75 0 1 0 1.06-1.061l-1.591-1.59Z" /></svg>
             Projekt Gehirn
         </button>

         <button @click="playAudio(isMapFocus ? 'map-close' : 'map-open'); isMapFocus = !isMapFocus; if(isMapFocus) { isMapMode = true; $dispatch('ai-toggle-brain-workspace', {open: false, silent: true}); isSecretMode = false; } if(window.innerWidth < 768) mobileMenuOpen = false;" class="px-3 py-2 border rounded-lg text-[10px] font-black uppercase tracking-widest transition-all backdrop-blur-md flex items-center gap-2 w-full md:w-auto justify-start md:justify-center"
         ...
         <button @click="playAudio(isSecretMode ? 'secret-close' : 'secret-open'); $dispatch('ai-toggle-secret-workspace', {open: !isSecretMode, silent: true}); if(window.innerWidth < 768) mobileMenuOpen = false;" class="px-3 py-2 border rounded-lg text-[10px] font-black uppercase tracking-widest transition-all backdrop-blur-md flex items-center gap-2 w-full md:w-auto justify-start md:justify-center"
         ...
         <button @click="playAudio(allowVoiceInterruption ? 'unclick' : 'click'); $dispatch('toggle-voice-interruption'); if(window.innerWidth < 768) mobileMenuOpen = false;" class="px-3 py-2 border rounded-lg text-[10px] font-black uppercase tracking-widest transition-all backdrop-blur-md flex items-center gap-2 w-full md:w-auto justify-start md:justify-center"
         ...
         <button @click="playAudio(showWorkspaceModal ? 'unclick' : 'click'); showWorkspaceModal = !showWorkspaceModal; if(window.innerWidth < 768) mobileMenuOpen = false;" class="px-3 py-2 border rounded-lg text-[10px] font-black uppercase tracking-widest transition-all backdrop-blur-md flex items-center gap-2 w-full md:w-auto justify-start md:justify-center"
             :class="showWorkspaceModal ? 'text-emerald-300 border-emerald-500/50 bg-emerald-900/30 hover:bg-emerald-800 shadow-[0_0_15px_rgba(16,185,129,0.3)]' : 'text-gray-500 border-gray-700 bg-gray-900/50 hover:text-gray-300 hover:bg-gray-800'">
         ...
         <button @click="playAudio(showDebugLog ? 'unclick' : 'click'); showDebugLog = !showDebugLog; if(window.innerWidth < 768) mobileMenuOpen = false;" class="px-3 py-2 border rounded-lg text-[10px] font-black uppercase tracking-widest transition-all backdrop-blur-md flex items-center gap-2 w-full md:w-auto justify-start md:justify-center"
