...
<main>
    ...
    <h3>Angebot für {{ $data['contact']['firma'] ?? $data['contact']['nachname'] }}</h3>

    <table class="table">
        <thead>
        <tr>
            <th width="50%">Artikel & Konfiguration</th>
            <th width="15%">Details</th>
            <th width="10%" class="text-right">Menge</th>
            <th width="10%" class="text-right">Einzelpreis<br><span style="font-size: 9px; font-weight: normal; color:#666;">(Brutto)</span></th>
            <th width="15%" class="text-right">Gesamt<br><span style="font-size: 9px; font-weight: normal; color:#666;">(Netto)</span></th>
        </tr>
        </thead>
        <tbody>
                    ...
                    @if(!empty($item['config']))
                        <div class="detail-block">
                            @if(!empty($item['config']['text']))
                                ...
                                <div class="detail-row"><span class="meta-label">Pos:</span> {{ $item['config']['text_pos_label'] ?? 'Manuell' }}</div>
                            @endif

                            @if(!empty($item['config']['logo_storage_path']))
                                <div class="detail-row" style="margin-top:8px; color:green;"><span class="meta-label">Logo:</span> Datei anbei</div>
                                <div class="detail-row"><span class="meta-label">Pos:</span> {{ $item['config']['logo_pos_label'] ?? 'Manuell' }}</div>
                            @endif
                        </div>
                    @endif
                </td>
    ...
    <table class="totals-table">
        <tr>
            <td width="60%"></td>
            <td class="totals-label">Summe (Netto):</td>
        ...
        <tr class="gross-row">
            <td></td>
            <td>Endsumme (Brutto):</td>
            <td>{{ $data['total_gross'] }} €</td>
        </tr>
    </table>

    <div style="margin-top: 50px; font-size: 11px; color: #555; border-top: 1px solid #eee; padding-top: 10px;">
        <strong>Kontaktdaten Kunde:</strong><br>
        ...
        @if(!empty($data['express']))
            <br><br><strong style="color:red;">EXPRESS-AUFTRAG (Wunschtermin: {{ \Carbon\Carbon::parse($data['deadline'])->format('d.m.Y') }})</strong>
        @endif
    </div>
</main>
