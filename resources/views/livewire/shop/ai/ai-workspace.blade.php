...
<div class="h-auto min-h-[calc(100dvh-4rem)] lg:h-[calc(100vh-6rem)] w-full font-mono text-[var(--theme-color)] flex flex-col pt-4 overflow-hidden relative">
    ...
    <div class="text-center mb-4 lg:mb-6 shrink-0 relative z-10 w-full px-4 lg:px-6">
        ...
        <p class="text-gray-400 text-xs font-bold uppercase tracking-widest mt-1">Multi-Agenten Arbeitsfläche (Gen-UI Kommunikation)</p>
    </div>
    ...
    <div class="flex-1 flex flex-col lg:flex-row gap-4 lg:gap-6 px-4 lg:px-6 pb-4 lg:pb-6 overflow-hidden relative"
        ...
        <div class="w-full lg:w-72 bg-gray-950 border border-gray-800 rounded-2xl p-4 flex flex-col shrink-0 z-10 shadow-xl shadow-[var(--theme-color-10)]">
            <h3 class="text-xs uppercase tracking-widest text-gray-400 mb-4 border-b border-gray-800 pb-2">Auftrag erstellen</h3>
            <form wire:submit="createTask" class="mb-6">
                <textarea wire:model="newTaskPrompt" 
                          placeholder="Anweisung oder Fragestellung eingeben..."
                          class="w-full bg-gray-900/50 backdrop-blur-xl border border-gray-800 rounded-xl p-3 text-sm text-[var(--theme-color)] placeholder:text-emerald-900 focus:outline-none focus:border-[var(--theme-color)] transition-colors resize-none h-24 shadow-inner"
                          @keydown.enter.prevent="$wire.createTask()"></textarea>
                <button type="submit" class="mt-2 w-full py-2 bg-[var(--theme-color-10)] text-[var(--theme-color)] border border-[var(--theme-color-30)] rounded-lg text-xs font-bold uppercase tracking-widest hover:bg-[var(--theme-color)] hover:text-black transition-all">
                    In den Raum werfen +
                </button>
            </form>

            <h3 class="text-xs uppercase tracking-widest text-gray-400 mb-2 lg:mb-4 border-b border-gray-800 pb-2">Bereite Agenten <span class="hidden lg:inline">(Ziehbar)</span></h3>
            <div class="lg:flex-1 flex lg:flex-col overflow-x-auto lg:overflow-x-hidden lg:overflow-y-auto space-x-3 lg:space-x-0 lg:space-y-3 custom-scrollbar pb-2 lg:pb-0 pr-0 lg:pr-2">
                @foreach($agents as $agent)
                    <div class="agent-draggable w-auto min-w-[150px] lg:min-w-0 bg-gray-900/50 backdrop-blur-xl border border-gray-800 rounded-xl p-3 flex items-center gap-3 cursor-grab lg:hover:border-[var(--theme-color-50)] transition-colors group shrink-0 lg:shrink"
                         draggable="true"
                        ...
                        <div class="w-10 h-10 rounded-full bg-gray-900 border border-gray-700 overflow-hidden shrink-0 flex items-center justify-center">
                            ...
                            @else
                                <span class="text-xs text-gray-500 group-hover:text-[var(--theme-color)] font-bold">{{ substr($agent->name, 0, 2) }}</span>
                            @endif
                        </div>
                ...
                @foreach($tasks as $task)
                    <div class="task-node w-full lg:w-80 bg-gray-950/90 backdrop-blur-md border {{ $task->status === 'completed' ? 'border-[var(--theme-color-50)] shadow-xl shadow-[var(--theme-color-10)]' : ($task->status === 'processing' ? 'border-cyan-500/50 shadow-[0_0_15px_rgba(6,182,212,0.1)] animate-pulse' : 'border-gray-800') }} rounded-2xl p-5 flex flex-col transition-all shrink-0"
                        ...
                        @if($task->status === 'completed' && $task->response_content)
                            <div class="mt-4 pt-4 border-t border-gray-800 text-xs text-gray-300 bg-[var(--theme-color-10)] rounded p-3 font-sans break-all">
    ...
    <style>
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb { background: var(--theme-color-20); border-radius: 4px; }
