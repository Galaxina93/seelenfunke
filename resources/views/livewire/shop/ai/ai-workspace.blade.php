...
@foreach($tasks as $task)
                        <div class="task-node relative w-full lg:w-80 {{ $task->parent_task_id ? 'ml-0 lg:ml-8 border-l-4 border-l-[var(--theme-color-50)]' : '' }} bg-gray-950/90 backdrop-blur-md border {{ $task->status === 'completed' ? 'border-[var(--theme-color-50)] shadow-xl shadow-[var(--theme-color-10)]' : ($task->status === 'processing' ? 'border-cyan-500/50 shadow-[0_0_15px_rgba(6,182,212,0.1)] animate-pulse-slow' : 'border-gray-800') }} rounded-2xl p-5 flex flex-col transition-all shrink-0"
                            ...
                            @if((isset($task->ui_metadata['execution_plan']) && count($task->ui_metadata['execution_plan']) > 0) || $task->response_content || isset($task->ui_metadata['work_log']))
                                <div wire:key="task-details-mobile-{{ $task->id }}-{{ $task->status }}" x-data="{ detailsOpen: {{ in_array($task->status, ['completed', 'failed']) ? 'false' : 'true' }} }" class="mt-4">
                                    ...
                                    <tbody class="divide-y divide-gray-800/60">
                                        ...
                                        <tr class="transition-colors hover:bg-gray-900/40 {{ $task->parent_task_id ? 'bg-gray-900/20' : '' }}"
                                            ...
                                            <td class="px-6 py-5 align-top w-1/2">
                                                ...
                                                <!-- DETAILS (Plan, Log, Response) -->
                                                @if((isset($task->ui_metadata['execution_plan']) && count($task->ui_metadata['execution_plan']) > 0) || $task->response_content || isset($task->ui_metadata['work_log']))
                                                    <div wire:key="task-details-{{ $task->id }}-{{ $task->status }}" x-data="{ detailsOpen: {{ in_array($task->status, ['completed', 'failed']) ? 'false' : 'true' }} }" class="mt-2">
                                                        ...
                                                        <div x-show="detailsOpen" x-collapse>
                                                            ...
                                                            <!-- WORK LOG (Terminal Style) -->
                                                            @if(isset($task->ui_metadata['work_log']) && count($task->ui_metadata['work_log']) > 0)
                                                                <div class="mb-3 relative z-10 w-full bg-black/60 p-2.5 rounded-lg border border-gray-800/80 shadow-inner overflow-hidden flex flex-col max-h-[200px]">
                                                                    <div class="flex items-center gap-2 mb-2 pb-1.5 border-b border-gray-800/50">
                                                                        <div class="w-2 h-2 rounded-full bg-red-500/50"></div>
                                                                        <div class="w-2 h-2 rounded-full bg-yellow-500/50"></div>
                                                                        <div class="w-2 h-2 rounded-full bg-green-500/50"></div>
                                                            ...
                                                            @if($task->response_content)
                                                                <div class="relative z-10" x-data="{ expandedResp: false }">
                                                                     <div class="text-[12px] text-gray-300 bg-[var(--theme-color-10)] rounded-lg p-3 font-sans break-words bg-opacity-40 relative border border-[var(--theme-color-20)]"
                                                                          :class="expandedResp ? 'max-h-none pb-8' : 'max-h-[70px] overflow-hidden'">
                                                                         @if(strlen($task->response_content) > 80)
                                                                             <div wire:ignore class="ai-markdown-content" x-init="if (window.renderAiMarkdown) { $el.innerHTML = window.renderAiMarkdown(@js($task->response_content)); } else { $el.innerText = @js($task->response_content); }"></div>
