                ...
                <div class="bg-gray-900 backdrop-blur-xl border border-gray-800 rounded-3xl p-6 hover:border-[color:var(--theme-color-50)] text-[color:var(--theme-color)] transition-all group relative overflow-hidden font-sans text-sm shadow-lg">
                    ...
                    <div class="space-y-3 pt-4 border-t border-gray-800 group-hover:border-gray-700 transition-colors">
                        ...
                        @foreach($catData['views'] as $viewName => $viewPath)
                            <div class="flex justify-between items-center bg-gray-950 p-3 rounded-xl border border-gray-800 transition-colors">
                                <span class="text-xs font-medium text-gray-400">{{ $viewName }}</span>
                                @if(str_contains($viewPath, 'livewire.'))
                                    <span class="text-xs font-mono text-[color:var(--theme-color)] bg-[color:var(--theme-color-10)] border border-[color:var(--theme-color-20)] px-2 py-1 rounded-lg">{{ $viewPath }}</span>
                                @else
                                    <span class="text-xs font-mono text-yellow-500 bg-yellow-500/10 border border-yellow-500/20 px-2 py-1 rounded-lg">{{ $viewPath }}</span>
                                @endif
                            </div>
                        @endforeach
                    </div>
