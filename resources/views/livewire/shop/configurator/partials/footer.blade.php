@if($context !== 'preview')
    <div class="p-4 border-t border-gray-200 bg-white z-30 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] shrink-0" x-data="{saved: false}" x-on:cart-updated.window="saved = true; setTimeout(() => saved = false, 6000)">
        <div class="max-w-4xl mx-auto space-y-4">

            <div @class(['p-3 rounded-xl border transition-all duration-200', 'bg-gray-50 border-gray-200' => !$errors->has('config_confirmed'), 'bg-red-50 border-red-200' => $errors->has('config_confirmed')])>
                <label class="flex items-start gap-3 cursor-pointer group">
                    <input type="checkbox" wire:model.live="config_confirmed" class="mt-1 w-4 h-4 text-primary rounded border-gray-300 focus:ring-primary">
            ...
            <div class="flex flex-col sm:flex-row gap-3 h-auto sm:h-14">
                ...
                <button wire:click="save" wire:loading.attr="disabled" @disabled(!$config_confirmed) :class="saved ? 'bg-green-600 hover:bg-green-700' : ({{$config_confirmed ? 'true' : 'false'}} ? 'bg-gray-900 hover:bg-black' : 'bg-gray-200 cursor-not-allowed text-gray-400')" class="flex-1 h-14 rounded-xl font-bold text-lg transition-all duration-300 flex items-center justify-center shadow-lg hover:shadow-xl disabled:shadow-none text-white relative overflow-hidden group">
                    @if($context !== 'template_admin')
            ...
            @if($context !== 'template_admin' && $context !== 'calculator')
                <div x-show="saved" x-transition:enter="transition ease-out duration-500" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="w-full pt-4" style="display: none;">
                    <a href="{{route('cart')}}" class="w-full border-2 border-primary text-gray-900 py-3 rounded-xl font-bold text-base flex items-center justify-center gap-2 transition-all duration-300 hover:bg-primary hover:text-white group">
