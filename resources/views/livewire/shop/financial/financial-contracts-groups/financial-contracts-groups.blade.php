<div x-data="{ draggingItemId: null }">
    <div class="min-h-screen bg-transparent pb-20 font-sans text-gray-300">
        ...
        {{-- Header --}}
        <div
            class="bg-gray-900/80 backdrop-blur-md shadow-2xl border-b border-gray-800 sticky top-0 z-30 transition-all duration-300">
        ...
        <div class="max-w-7xl mx-auto px-4 mt-8 md:mt-12 space-y-8 md:space-y-10 animate-fade-in-up"
            ...
            @if($missingContracts->count() > 0)
                ...
                <div
                    class="bg-gray-900/80 backdrop-blur-md rounded-[2rem] shadow-[0_0_30px_rgba(239,68,68,0.05)] border border-red-500/20 overflow-hidden transition-all duration-300"
                    x-data="{ expanded: false }">
                    <div
                        class="bg-red-900/10 px-6 sm:px-8 py-5 border-b border-red-500/20 flex justify-between items-center cursor-pointer hover:bg-red-900/20 transition-colors shadow-inner"
                        @click="expanded = !expanded"
                    >
                        <div class="flex items-center gap-4">
                            <div
                                class="p-2.5 bg-red-500/10 text-red-500 border border-red-500/20 rounded-xl shadow-[0_0_15px_rgba(239,68,68,0.2)] animate-pulse shrink-0">
                            ...
                            <div>
                                <h3 class="text-base sm:text-lg font-serif font-bold text-red-400 flex flex-wrap items-center gap-3 tracking-wide">
                                    ...
                                    {{-- Counter Anzeige --}}
                                    <span
                                        class="bg-red-500/20 text-red-300 border border-red-500/30 text-[9px] font-black tracking-widest px-2.5 py-0.5 rounded-md uppercase">{{ $missingContracts->count() }} / {{ $totalItems }}</span>
                                </h3>
                                <p class="text-[10px] sm:text-xs text-red-300/80 font-medium mt-1">Bei folgenden
                                    Kostenstellen wurde noch kein Vertrag oder Beleg hinterlegt.</p>
                            </div>
                        </div>

                        {{-- Chevron Icon für Collapse --}}
                        <div class="text-red-500/50 transition-transform duration-300 shrink-0 ml-4"
                    ...
                    <div x-show="expanded" x-collapse>
                        <div class="overflow-x-auto w-full no-scrollbar">
                            <table class="w-full text-sm text-left min-w-[500px]">
                                <thead
                                    class="bg-gray-950/50 text-[10px] text-gray-500 font-black uppercase tracking-widest border-b border-gray-800">
                                <tr>
                                    <th class="px-6 sm:px-8 py-4">Kostenstelle</th>
                                    <th class="px-4 py-4">Gruppe</th>
                                    <th class="px-6 sm:px-8 py-4 text-right">Aktion</th>
                                </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-800/50 bg-transparent">
                                @foreach($missingContracts as $missingItem)
                                    <tr class="hover:bg-gray-800/30 transition-colors group">
                                        <td class="px-6 sm:px-8 py-5 font-bold text-white tracking-wide">{{ $missingItem->name }}</td>
                                            ...
                                            @if($uploadingMissingItemId === $missingItem->id)
                                                ...
                                                <div class="flex items-center justify-end gap-3">
                                                    <input type="file" wire:model="quickUploadFile"
                                                           class="w-full max-w-[200px] text-[9px] text-gray-400 file:mr-2 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-[9px] file:font-black file:uppercase file:tracking-widest file:bg-red-500/10 file:text-red-400 file:border file:border-red-500/20 hover:file:bg-red-500/20 transition-all cursor-pointer">

                                                    <button wire:click="saveQuickUpload" wire:loading.attr="disabled"
                                                            class="bg-red-500 text-gray-900 px-4 py-2 rounded-xl hover:bg-red-400 transition-all shadow-[0_0_15px_rgba(239,68,68,0.2)] disabled:opacity-50 text-[9px] font-black uppercase tracking-widest shrink-0">
                                                        <span wire:loading.remove wire:target="saveQuickUpload">Speichern</span>
                                                        <span wire:loading wire:target="saveQuickUpload"
                                                    ...
                                                    <button wire:click="cancelQuickUpload"
                                                            class="text-gray-500 hover:text-red-400 px-2 transition-colors">
                                                ...
                                                @error('quickUploadFile') <span
                                                    class="text-[9px] font-bold text-red-400 uppercase tracking-widest block mt-2">{{ $message }}</span> @enderror
                                            @else
                                                <button wire:click="startQuickUpload('{{ $missingItem->id }}')"
                                                        class="text-[9px] bg-red-500/10 text-red-400 px-4 py-2 rounded-xl hover:bg-red-500/20 hover:text-red-300 transition-all font-black uppercase tracking-widest shadow-inner border border-red-500/20">
                                                    Jetzt hochladen
                                                </button>
                                            @endif
                                        </td>
            ...
            @else
                {{-- GRÜNE SUCCESS BOX --}}
                <div
                    class="bg-gray-900/80 backdrop-blur-md rounded-[2rem] shadow-[0_0_30px_rgba(16,185,129,0.05)] border border-emerald-500/20 overflow-hidden animate-fade-in-down">
                    <div class="bg-emerald-900/10 px-6 sm:px-8 py-5 flex items-center gap-4 shadow-inner">
                        <div
                            class="p-2.5 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-xl shadow-[0_0_15px_rgba(16,185,129,0.2)] shrink-0">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div>
                            <h3 class="text-base sm:text-lg font-serif font-bold text-emerald-400 tracking-wide drop-shadow-[0_0_8px_currentColor]">
                                Vertrags-Check: Alle Unterlagen liegen vor</h3>
                            <p class="text-[10px] sm:text-xs text-emerald-300/70 font-medium mt-0.5">Es wurden zu allen
                                Kostenstellen entsprechende Belege oder Verträge hinterlegt.</p>
                        </div>
                    </div>
                </div>
            @endif
            ...
            <div
                class="bg-gray-900/80 backdrop-blur-md rounded-[2.5rem] shadow-2xl border border-gray-800 p-6 sm:p-10 flex flex-col md:flex-row items-center gap-8 relative overflow-hidden"
                wire:ignore>
                <div
                    class="absolute top-0 right-0 w-64 h-64 bg-primary/5 rounded-full blur-[80px] -translate-y-1/2 translate-x-1/3 pointer-events-none"></div>
                <div class="w-full md:w-1/3 relative z-10 text-center md:text-left">
                    ...
                    <p class="text-xs text-gray-400 leading-relaxed font-medium">Übersicht der monatlichen Volumina nach
                        Gruppen. <br class="hidden md:block mt-2"> <span
                            class="text-emerald-400 font-bold drop-shadow-[0_0_5px_currentColor]">Grün = Einnahmen</span>,
                        <span
                            class="text-red-400 font-bold drop-shadow-[0_0_5px_currentColor]">Rot = Ausgaben</span>.</p>
                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8"
                ...
                @foreach($groups as $group)
                    ...
                    >
                        ...
                        <div
                            class="w-full py-2 bg-gray-950 flex items-center justify-center cursor-grab active:cursor-grabbing border-b border-gray-800 opacity-50 hover:opacity-100 transition-opacity shadow-inner"
                        ...
                        @if($activeGroupId === $group->id)
                            <div
                                class="border-t border-gray-800 bg-gray-950/50 p-5 sm:p-8 animate-fade-in-down shadow-inner"
                                @dragstart.stop>
                                    ...
                                    @foreach($group->items as $item)
                                        ...
                                        >
                                            @if($editingItemId === $item->id)
                                                {{-- EDIT COSTITEM --}}
                                                <div class="p-5 sm:p-6 bg-gray-800/30">
