    ...
    <div class="bg-gray-900/80 backdrop-blur-xl p-6 sm:p-8 rounded-[2.5rem] shadow-2xl border border-gray-800 animate-fade-in-up transition-colors hover:border-gray-700 w-full min-w-0">
        ...
        <div class="flex flex-wrap lg:flex-nowrap items-start lg:items-center justify-between gap-4 sm:gap-5 transition-all" :class="open ? 'mb-6 border-b border-gray-800 pb-5' : ''">
            ...
            <div @click="open = !open" class="flex items-center gap-3 sm:gap-4 cursor-pointer group flex-1 min-w-[200px] w-full lg:w-auto">
                ...
                <h3 class="text-lg sm:text-xl font-serif font-bold text-white tracking-wide group-hover:text-[var(--theme-color)] transition-colors truncate">Kategorien</h3>
                ...
                <div class="pt-2">
                    ...
                    <div class="mb-5 relative group">
                        <input type="text"
                               ...
                               class="w-full pl-12 pr-4 py-3.5 rounded-xl border border-gray-800 bg-gray-950 text-sm text-white focus:bg-black focus:border-[var(--theme-color)] focus:ring-2 focus:ring-[var(--theme-color-20)] transition-all shadow-inner outline-none placeholder-gray-600">
                        ...
                        @forelse($availableCategories as $category)
                            ...
                            <button
                                ...
                                class="group relative flex items-center gap-4 p-4 rounded-2xl border text-left transition-all duration-300 shadow-inner min-w-0
                                       {{ $isSelected
                                          ? 'border-[var(--theme-color)] bg-[var(--theme-color-10)] shadow-[inset_0_0_15px_rgba(197,160,89,0.2)]'
                                ...
                                <div class="flex-shrink-0 w-6 h-6 rounded-lg border flex items-center justify-center transition-all shadow-inner
                                            {{ $isSelected ? 'bg-[var(--theme-color)] border-[var(--theme-color)] shadow-[0_0_10px_rgba(197,160,89,0.5)]' : 'border-gray-700 bg-gray-900 group-hover:border-gray-500' }}">
                                    @if($isSelected)
                            ...
                            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-gray-950 border border-gray-800 rounded-2xl shadow-inner hover:border-gray-700 transition-colors group gap-4 min-w-0" wire:key="cat-{{ $catId }}">
                                ...
                                @if($editingCategoryId === $catId)
                                    <div class="flex flex-1 w-full items-center gap-2 sm:gap-3 mr-0 sm:mr-3 animate-fade-in min-w-0">
                                        <input type="text"
                                               ...
                                               class="w-full flex-1 px-4 py-2.5 text-sm font-bold border border-[var(--theme-color)] bg-gray-900 text-white rounded-xl focus:ring-2 focus:ring-[var(--theme-color-30)] outline-none shadow-inner min-w-0">
                                ...
                                @else
                                    <div class="flex items-center gap-3 min-w-0 flex-1">
                                        <div class="w-2 h-2 rounded-full shrink-0 {{ in_array($catId, $selectedCategories) ? 'bg-[var(--theme-color)] shadow-[0_0_8px_currentColor]' : 'bg-gray-700' }}"></div>
                                    ...
                                    <div class="flex items-center gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity w-full sm:w-auto justify-end shrink-0 mt-2 sm:mt-0">
                                        <button wire:click="startEditing({{ $catId }}, '{{ addslashes($catName) }}')" class="p-2.5 text-gray-500 hover:text-[var(--theme-color)] bg-gray-900 border border-gray-800 hover:border-[var(--theme-color-30)] rounded-xl transition-all shadow-inner" title="Umbenennen">
