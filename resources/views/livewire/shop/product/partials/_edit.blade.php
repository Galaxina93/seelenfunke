{{-- Sticky Header --}}
<div
    class="sticky top-0 z-30 bg-white/95 backdrop-blur border-b border-gray-200 px-6 py-3 flex items-center justify-between shadow-sm">
    <div class="flex items-center gap-4">
        <button wire:click="backToList" class="p-2 hover:bg-gray-100 rounded-full text-gray-500 transition"
                title="Zurück">
        ...
        <div>
            ...
            <p class="text-xs text-primary font-bold uppercase tracking-wider">Schritt {{ $currentStep }}
                von {{ $totalSteps }}</p>
        </div>
    </div>
    <div class="flex items-center gap-4">
        ...
        @endif
        <button wire:click="save" class="text-gray-500 hover:text-gray-900 text-sm font-medium px-2">Produkt speichern
        </button>
    </div>
</div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-12 gap-12 items-start">
        ...
        <div class="xl:col-span-7 space-y-8">
            ...
            <div class="flex justify-between pt-8">
                <button @if($currentStep === 1) disabled @else wire:click="prevStep"
                        @endif class="px-6 py-3 rounded-full font-bold text-gray-500 hover:bg-gray-100 hover:text-gray-900 disabled:opacity-30 disabled:cursor-not-allowed transition">
                    &larr; Zurück
                </button>
                @if($currentStep === $totalSteps)
                    <button wire:click="finish" @if(!$canProceed) disabled
                            @endif class="bg-primary text-white px-10 py-3 rounded-full font-bold shadow-lg shadow-primary/30 hover:bg-white hover:text-primary border border-transparent hover:border-primary transition transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none">
                        Fertigstellen & Veröffentlichen
                    </button>
                @else
                    <button wire:click="nextStep" @if(!$canProceed) disabled
                            @endif class="px-8 py-3 rounded-full font-bold shadow-lg transition transform hover:-translate-y-0.5 {{ $canProceed ? 'bg-gray-900 text-white hover:bg-black' : 'bg-gray-300 text-gray-500 cursor-not-allowed' }}">
                        Weiter &rarr;
                    </button>
                @endif
            </div>
            ...
            <div class="sticky top-32 space-y-6">
                ...
                <div
                    ...
                    <div class="aspect-square bg-gray-50 relative overflow-hidden">
                        ...
                        @else
                            <div
                                class="absolute inset-0 flex items-center justify-center text-gray-300 font-serif italic">
                                Vorschau
                            </div>
                        @endif
                    </div>
                    <div class="p-8">
                        ...
                        <div class="flex items-baseline gap-4 mb-6">
                            ...
                            <span class="text-xs text-gray-400">
                                        {{-- Zugriff direkt über das Model --}}
                                {{ $product->tax_included ? 'inkl.' : 'zzgl.' }} MwSt.

                                        {{-- Nutzt den Accessor getTaxRateAttribute() aus dem Model --}}
                                        ({{ number_format($product->tax_rate, 0) }}%)
                                    </span>
                        </div>
                        <div
                            class="grid grid-cols-2 gap-y-4 gap-x-8 text-sm text-gray-500 mb-8 pt-6 border-t border-gray-100">
                            <div><span
                                    class="font-bold text-gray-900 block mb-1">Material</span>{{ $productAttributes['Material'] ?? '-' }}
                            </div>
                            <div><span
                                    class="font-bold text-gray-900 block mb-1">Größe</span>{{ $productAttributes['Größe'] ?? '-' }}
                            </div>
                            @if(!empty($productAttributes['Farbe']))
                                <div><span
                                        class="font-bold text-gray-900 block mb-1">Farbe</span>{{ $productAttributes['Farbe'] }}
                                </div>
                            @endif
                            @if($track_quantity && $quantity > 0)
                                <div class="col-span-2 text-green-600 font-bold text-xs mt-2 flex items-center gap-1">
                                    <span class="w-2 h-2 bg-green-500 rounded-full inline-block"></span> Auf Lager
                                    ({{ $quantity }})
                                </div>
                            @endif
                        </div>
                        <button
                            class="w-full bg-primary text-white font-bold py-4 rounded-full shadow-lg shadow-primary/20 flex items-center justify-center gap-2 opacity-50 cursor-default">
