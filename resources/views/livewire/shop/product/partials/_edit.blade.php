<div class="w-full" x-data="{ showPreview: false }" wire:key="product-edit-wrapper-{{ $product->id ?? 'new' }}">
    <div class="hidden xl:col-span-7 xl:col-span-12"></div>

    <div class="sticky top-0 z-40 bg-gray-900/80 backdrop-blur-xl border-b border-gray-800 px-4 sm:px-8 py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between shadow-2xl gap-4">
        ...
        <div class="flex items-center gap-4 sm:gap-6 w-full sm:w-auto justify-end">
            ...
            @endif
            <button wire:click="save" class="text-[10px] font-black uppercase tracking-widest text-gray-500 hover:text-white border-b border-gray-600 hover:border-white pb-0.5 transition-colors shrink-0">Speichern</button>
        </div>
    </div>

    <div class="max-w-[1800px] mx-auto p-4 sm:p-6 lg:p-10 font-sans antialiased text-gray-300">
        <div class="flex flex-col lg:flex-row gap-6 lg:gap-12 items-start w-full">

            <div class="w-full transition-all duration-500 space-y-6 lg:space-y-10 {{ $currentStep === 4 ? 'lg:!w-full' : '' }}"
                 :class="showPreview ? 'lg:w-7/12' : 'lg:w-full'">

                <div class="flex items-start gap-2 sm:gap-4 mb-8 overflow-x-auto no-scrollbar pb-2">
                    @php
                        $stepLabels = [
                            1 => 'Basisdaten',
                            2 => 'Medien',
                ...
                <div class="flex flex-col sm:flex-row justify-between items-center pt-8 border-t border-gray-800 mt-8 gap-4 sm:gap-0">
                    ...
                    <div class="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
                        ...
                        @php
                            $isLastStep = ($type === 'physical' && $currentStep === 4) || ($type !== 'physical' && $currentStep === 3);
                        @endphp
            ...
            @if($currentStep !== 4)
                <div class="w-full lg:w-5/12 relative transition-all duration-500"
                     x-show="showPreview"
                    ...
                    <div class="sticky top-32 space-y-6">
                        <h3 class="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] text-center flex items-center justify-center gap-3">
                            <span class="w-8 h-px bg-gray-800"></span> Shop Vorschau <span class="w-8 h-px bg-gray-800"></span>
                        </h3>
                        ...
                        <div class="bg-gray-900/80 backdrop-blur-md rounded-[2.5rem] shadow-2xl border border-gray-800 overflow-hidden transform transition hover:border-gray-700">
                            <div class="aspect-square bg-gray-950 relative overflow-hidden shadow-inner p-2">
                                <div class="w-full h-full rounded-[2rem] overflow-hidden relative border border-gray-800/50">
                                    @php
                                        // Sicheres Auslesen des Vorschaubilds
                                        $shopPreviewImage = null;
                                        if (!empty($product->media_gallery) && is_array($product->media_gallery) && count($product->media_gallery) > 0) {
                                            $firstMedia = $product->media_gallery[0];
                                            $shopPreviewImage = is_array($firstMedia) ? ($firstMedia['path'] ?? null) : $firstMedia;
                                        } elseif (!empty($product->preview_image_path)) {
                                            $shopPreviewImage = $product->preview_image_path;
                                        }
                                    @endphp

                                    @if($shopPreviewImage)
                                        <img src="{{ asset('storage/' . $shopPreviewImage) }}" class="w-full h-full object-cover">
                                    @else
                                        <div class="absolute inset-0 flex flex-col items-center justify-center text-gray-700 bg-gray-900 shadow-inner">
                                            ...
                                            <span class="font-serif italic text-lg">Kein Bild hochgeladen</span>
                                        </div>
                                    @endif
                                </div>
                                <div class="absolute top-6 right-6 bg-gray-900/90 backdrop-blur-md px-4 py-2 rounded-xl text-[9px] font-black text-gray-300 shadow-lg border border-gray-800 uppercase tracking-widest">
                                    @if($type === 'physical') Physisch
                                    @elseif($type === 'digital') Digital
                                    @else Service
                                    @endif
                                </div>
                            </div>
                                ...
                                <div class="grid grid-cols-2 gap-y-5 gap-x-8 text-sm text-gray-400 mb-8 pt-8 border-t border-gray-800">
                                    ...
                                    @if($track_quantity && $quantity > 0)
                                        <div class="col-span-2 text-emerald-400 font-bold text-xs mt-2 flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 px-4 py-2.5 rounded-xl shadow-inner w-fit">
                                            ...
                                            @if($type === 'service') Plätze verfügbar @else Auf Lager @endif ({{ $quantity }})
                                        </div>
                                    @endif
                                </div>
