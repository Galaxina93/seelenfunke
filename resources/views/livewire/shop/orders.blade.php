<div class="min-h-screen bg-gray-50 p-4 md:p-6">
    ...
    @if(!$selectedOrderId)
        ...
        <div class="bg-white border-x border-b border-gray-200 shadow-sm rounded-b-xl overflow-hidden">
            ...
            {{-- DESKTOP TABELLE (Sichtbar ab Tablet/Desktop) --}}
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    @forelse($orders as $order)
                        <tr class="hover:bg-gray-50/50 transition-colors group text-sm cursor-pointer" wire:click="openDetail('{{ $order->id }}')">
                            <td class="px-6 py-4 font-mono font-bold text-gray-900 text-primary hover:underline whitespace-nowrap">
                                {{ $order->order_number }}
                            </td>
            ...
            {{-- MOBILE KARTEN (Sichtbar nur unter Tablet-Breite) --}}
            <div class="md:hidden divide-y divide-gray-100">
                @forelse($orders as $order)
                    <div wire:click="openDetail('{{ $order->id }}')" class="p-4 active:bg-gray-50 transition-colors cursor-pointer">
                        ...
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold uppercase {{ $statusColors[$order->status] ?? 'bg-gray-100' }}">
                            {{ $statusMap[$order->status] ?? $order->status }}
                        </span>
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold uppercase {{ $paymentColors[$order->payment_status] ?? 'bg-gray-100' }}">
                        ...
                        <div class="flex justify-between items-end">
                            <div class="truncate pr-4">
                                <div class="font-bold text-gray-900 text-sm truncate">{{ $order->customer_name }}</div>
                                <div class="text-[11px] text-gray-400 truncate">{{ $order->email }}</div>
                            </div>
                            <div class="text-right">
                                <div class="text-xs text-gray-400 mb-0.5">Gesamtbetrag</div>
                                <div class="font-bold text-gray-900 text-base">{{ number_format($order->total_price / 100, 2, ',', '.') }} €</div>
                            </div>
                        </div>
                    </div>
                @empty
