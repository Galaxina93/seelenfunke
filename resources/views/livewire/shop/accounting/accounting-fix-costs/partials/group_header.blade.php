        ...
        <div class="flex-1 min-w-0">
            @if($editingGroupId === $group->id)
                <div class="flex items-center gap-2 sm:gap-3" @click.stop>
                    <input type="text" wire:model="tempGroupName" class="text-sm font-bold border border-gray-700 bg-gray-950 text-white rounded-xl px-3 py-2 w-full max-w-[200px] focus:ring-2 focus:ring-[var(--theme-color-50)] focus:border-[var(--theme-color)] outline-none shadow-inner" wire:keydown.enter="updateGroup">
            ...
            @else
                <span class="font-serif font-bold text-lg sm:text-xl text-white tracking-wide cursor-pointer group-hover/header:text-[var(--theme-color)] transition-colors flex items-center gap-3 group/edit" wire:click.stop="editGroup('{{ $group->id }}')">
                    <span class="truncate">{{ $group->name }}</span>
                    <svg class="w-4 h-4 text-gray-600 opacity-0 group-hover/edit:opacity-100 hover:text-[var(--theme-color)] transition-all shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                </span>
            @endif
    ...
    <div class="flex items-center gap-4 sm:gap-6 shrink-0">
        ...
        <div class="flex items-center gap-1 sm:gap-2">
            ...
            <div class="text-gray-500 group-hover/header:text-white transition-colors p-2 cursor-pointer bg-gray-900 rounded-full border border-gray-800 shadow-inner group-hover/header:border-gray-600">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 transform transition-transform duration-300 {{ $activeGroupId === $group->id ? 'rotate-180 text-[var(--theme-color)]' : '' }}" viewBox="0 0 20 20" fill="currentColor">
