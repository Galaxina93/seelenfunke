<div x-data="{ open: false }" class="bg-gray-900/80 backdrop-blur-md rounded-[2.5rem] shadow-2xl border border-gray-800 overflow-hidden relative group">
    <div @click="open = !open" class="p-6 md:p-8 flex items-center justify-between cursor-pointer hover:bg-gray-800/50 transition-colors">
        <div class="flex items-center gap-4">
            <div class="p-2.5 bg-[var(--theme-color-10)] border border-[var(--theme-color-20)] text-[var(--theme-color)] rounded-xl shadow-inner">
    ...
    <div x-show="open" x-collapse style="display: none;">
        <div class="p-6 md:p-8 pt-0 border-t border-gray-800 relative">
            <div class="absolute top-0 right-0 w-64 h-64 bg-[var(--theme-color-10)] rounded-full blur-[80px] -mr-16 -mt-16 pointer-events-none opacity-50"></div>

            <div class="mt-6 mb-10 relative z-10">
                <div class="flex flex-col md:flex-row gap-4 bg-gray-950 p-2.5 rounded-[1.5rem] border border-gray-800 focus-within:ring-2 focus-within:ring-[var(--theme-color-30)] focus-within:border-[var(--theme-color)] transition-all shadow-inner">
                    ...
                    <button
                        ...
                        class="bg-[var(--theme-color)]/90 border border-orange-400/50 hover:bg-[var(--theme-color)] text-white font-black uppercase tracking-widest text-[10px] py-3.5 px-8 rounded-xl shadow-[0_0_20px_rgba(249,115,22,0.2)] transition-all transform hover:scale-[1.02] flex items-center justify-center gap-3 whitespace-nowrap"
                    >
                        <span>Erstellen</span>
            ...
            <div x-data="{
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="bg-gray-950/50 border border-gray-800 rounded-2xl p-6">
                        <div class="flex items-center gap-3 mb-6 pb-4 border-b border-gray-800">
                            <div class="p-2 bg-[var(--theme-color-10)] text-[var(--theme-color)] rounded-lg shadow-inner">
                        ...
                        <div x-ref="businessList" class="space-y-3 min-h-[100px]">
                            @foreach($this->manageableCategories->where('is_business', true) as $cat)
                                <div data-id="{{ $cat->id }}" class="group/cat cursor-grab active:cursor-grabbing relative bg-gray-950 border border-gray-800 rounded-xl p-3 shadow-inner hover:border-[var(--theme-color-50)] hover:shadow-[0_0_15px_rgba(249,115,22,0.1)] transition-all duration-300" wire:key="cat-{{ $cat->id }}">
                                    @include('livewire.shop.accounting.partials.category_item_inner', ['cat' => $cat])
                                </div>
                            @endforeach
