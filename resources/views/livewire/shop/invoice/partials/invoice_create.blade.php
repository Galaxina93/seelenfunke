            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border {{ $errors->has('manualInvoice.items') ? 'border-red-300' : 'border-gray-200' }} overflow-x-auto">
                ...
                <table class="w-full text-sm">
                    ...
                    <tr class="text-left text-xs font-bold uppercase text-gray-500 border-b">
                        ...
                        <th class="pb-2 text-center w-20">Menge*</th>
                        <th class="pb-2 text-right w-32">Preis Netto*</th>
                        <th class="pb-2 text-center w-24">USt.</th>
                        <th class="pb-2 text-right w-32">Betrag</th>
            ...
            <div class="sticky top-6 scale-[0.85] xl:scale-90 origin-top">
                <div class="bg-white shadow-2xl p-8 xl:p-10 min-h-[297mm] w-[210mm] mx-auto text-sm text-gray-800 border flex flex-col relative">
                    ...
                    <table class="w-full mb-8 border-collapse">
                        ...
                        <tr>
                            <th class="py-3 px-4 text-left w-12 rounded-tl-lg">Pos</th>
                            <th class="py-3 px-4 text-left">Bezeichnung</th>
                            <th class="py-3 px-4 text-center w-20">Menge</th>
                            <th class="py-3 px-4 text-right w-32">Einzel Brutto</th>
                            <th class="py-3 px-4 text-right w-32 rounded-tr-lg">Gesamt</th>
                        </tr>
                        </thead>
                        <tbody class="text-xs">
                        @foreach($manualInvoice['items'] as $index => $item)
                            <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                                <td class="py-4 px-4 text-gray-400 font-mono align-top">{{ str_pad($index + 1, 2, '0', STR_PAD_LEFT) }}</td>
                                <td class="py-4 px-4 align-top">
                                    <div class="text-gray-900 font-bold uppercase tracking-tight">{{ $item['product_name'] ?: 'Artikelliste...' }}</div>
                                    @php
                                        $config = is_object($item) ? ($item->configuration ?? null) : ($item['configuration'] ?? null);
                                    @endphp
                                    @if($config)
                                        <div class="text-[9px] text-gray-500 italic mt-1 leading-tight">
                                ...
                                <td class="py-4 px-4 text-right align-top whitespace-nowrap">{{ number_format($item['unit_price'] ?: 0, 2, ',', '.') }} €</td>
                                <td class="py-4 px-4 text-right align-top font-bold text-gray-900 whitespace-nowrap">{{ number_format(($item['unit_price'] ?: 0) * ($item['quantity'] ?: 1), 2, ',', '.') }} €</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
                    ...
                    {{-- Summen Vorschau --}}
                    <div class="w-[300px] ml-auto space-y-2 bg-gray-50 p-6 rounded-2xl border border-gray-100 mb-8 text-right">
                        <div class="flex justify-between items-center text-gray-500 text-[10px] uppercase font-bold">
                            <span>Gesamt Netto</span>
                            <span class="whitespace-nowrap">{{ number_format($totalsPreview['net'], 2, ',', '.') }} €</span>
                        </div>
                        @if(!shop_setting('is_small_business', false))
                            <div class="flex justify-between items-center text-gray-500 text-[10px] uppercase font-bold">
                                <span>Umsatzsteuer</span>
                                <span class="whitespace-nowrap">{{ number_format($totalsPreview['tax'], 2, ',', '.') }} €</span>
                            </div>
                        @endif
                        @if($manualInvoice['shipping_cost'] > 0)
                            <div class="flex justify-between items-center text-gray-500 text-[10px] uppercase font-bold">
                                <span>Versandkosten</span>
                                <span class="whitespace-nowrap">{{ number_format((float)$manualInvoice['shipping_cost'], 2, ',', '.') }} €</span>
                            </div>
                        @endif
                        @if($manualInvoice['volume_discount'] > 0)
                            <div class="flex justify-between items-center text-red-500 text-[10px] uppercase font-bold italic">
                                <span>Mengenrabatt</span>
                                <span class="whitespace-nowrap">-{{ number_format((float)$manualInvoice['volume_discount'], 2, ',', '.') }} €</span>
                            </div>
                        @endif
                        @if($manualInvoice['discount_amount'] > 0)
                            <div class="flex justify-between items-center text-red-500 text-[10px] uppercase font-bold italic">
                                <span>Gutschein</span>
                                <span class="whitespace-nowrap">-{{ number_format((float)$manualInvoice['discount_amount'], 2, ',', '.') }} €</span>
                            </div>
                        @endif
                        <div class="flex justify-between items-center font-black text-2xl border-t border-gray-200 pt-4 mt-2 text-gray-900 tracking-tighter">
                            <span>Gesamtbetrag</span>
                            <span class="text-primary whitespace-nowrap">{{ number_format($totalsPreview['gross'], 2, ',', '.') }} €</span>
                        </div>
                        @if(shop_setting('is_small_business', false))
                            <div class="pt-2 text-[8px] text-gray-400 italic uppercase leading-tight">Umsatzsteuerfrei aufgrund der Kleinunternehmerregelung gemäß § 19 UStG.</div>
                        @endif
                    </div>
                    ...
                    <div class="mt-auto pt-8 border-t border-gray-100 text-center text-gray-400">
                        <p class="text-[10px] leading-relaxed">
                            <strong>{{ shop_setting('owner_name') }}</strong> | Inh. {{ shop_setting('owner_proprietor') }}<br>
                            {{ shop_setting('owner_street') }}, {{ shop_setting('owner_city') }}<br>
                            {{ shop_setting('owner_email') }} | {{ str_replace(['http://', 'https://'], '', shop_setting('owner_website')) }}
                        </p>
                    </div>
                </div>
            </div>
