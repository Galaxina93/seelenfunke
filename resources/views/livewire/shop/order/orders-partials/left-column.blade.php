...
<div
    class="w-full lg:w-1/2 h-1/2 lg:h-full overflow-y-auto border-b lg:border-b-0 border-r-0 lg:border-r border-gray-200 bg-white custom-scrollbar z-10">
    <div class="p-4 md:p-6 space-y-6 md:space-y-8">
        ...
        <div class="grid grid-cols-1 xl:grid-cols-2 gap-4 md:gap-6">
            ...
            <div class="bg-gray-50 p-4 rounded-xl border border-gray-100">
                ...
                <div class="text-sm text-gray-900 leading-snug">
                                    <span
                                        class="font-bold">{{ $this->selectedOrder->billing_address['first_name'] }} {{ $this->selectedOrder->billing_address['last_name'] }}</span><br>
            ...
            <div @class(['p-4 rounded-xl border', $isDifferent ? 'bg-amber-50 border-amber-200 shadow-sm' : 'bg-gray-50 border-gray-100'])>
                <h3 class="text-xs font-bold uppercase text-gray-500 mb-2 flex items-center gap-1">
                    ...
                    @if($isDifferent)
                        <span
                            class="ml-auto text-[9px] bg-amber-500 px-1.5 py-0.5 rounded-full font-black tracking-widest uppercase animate-pulse">Abweichend!</span>
                    @endif
                </h3>
                <div class="text-sm text-gray-900 leading-snug">
                    @if($this->selectedOrder->shipping_address)
                        <span
                            class="font-bold">{{ $this->selectedOrder->shipping_address['first_name'] }} {{ $this->selectedOrder->shipping_address['last_name'] }}</span>
                        <br>
            ...
            <div class="bg-gray-50 p-4 rounded-xl border border-gray-100 xl:col-span-2">
                ...
                <div class="space-y-2 text-sm">
                    <div class="flex justify-between">
                        ...
                        <span
                            class="font-bold {{ $this->selectedOrder->payment_status == 'paid' ? 'text-green-600' : 'text-yellow-600' }}">
                ...
                @if($selectedOrder->status === 'cancelled' && !empty($selectedOrder->cancellation_reason))
                    <div class="bg-green-50 border border-green-200 rounded-xl p-5 transition-all">
                        <div class="flex items-start gap-3 mb-3">
                            ...
                            <div>
                                <h4 class="font-bold text-green-800 text-sm">Stornierungsgrund
                                    gespeichert</h4>
                                <p class="text-xs text-green-700">Dieser Grund ist hinterlegt und für
                                    den Kunden sichtbar.</p>
                            </div>
                        </div>
                ...
                @else
                    <div class="bg-red-50 border border-red-200 rounded-xl p-5 transition-all">
                        <div class="flex items-start gap-3 mb-3">
                            ...
                            <div>
                                <h4 class="font-bold text-red-700 text-sm">Stornierungsgrund
                                    erforderlich</h4>
                                <p class="text-xs text-red-600">Bitte geben Sie einen Grund an, um die
                                    Stornierung abzuschließen.</p>
                            </div>
                        </div>
                    ...
                    >
                        ...
                        <div class="flex justify-between items-start">
                            <div class="flex items-center gap-4">
                                <div
                                    class="h-14 w-14 bg-white rounded-lg border border-gray-100 overflow-hidden flex-shrink-0 flex items-center justify-center">
                                    @php
                                ...
                                <div>
                                    <div
                                        class="font-bold text-gray-900 text-sm">{{ $item->product_name }}</div>
                                    <div class="text-xs text-gray-500 mt-0.5">{{ $item->quantity }}Stück
                                        á {{ number_format($item->unit_price / 100, 2, ',', '.') }} €
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="text-right">
                                <div
                                    class="font-mono font-bold text-gray-900 text-sm">{{ number_format($item->total_price / 100, 2, ',', '.') }}
                                    €
                                </div>
                                @if($selectedOrderItemId == $item->id)
                                    <div
                                        class="text-[10px] text-primary font-bold mt-1 bg-white px-2 py-0.5 rounded-full shadow-sm inline-block">
                                        WIRD ANGEZEIGT
                                    </div>
                                @else
                                    <div
                                        class="text-[10px] text-gray-400 mt-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        Anzeigen &rarr;
                                    </div>
                                @endif
                            </div>
                        ...
                        @if(!empty($conf))
                            <div
                                class="mt-3 pt-3 border-t border-gray-200/60 grid grid-cols-1 xl:grid-cols-2 gap-4 text-xs">

                                {{-- 1. Gravurtext --}}
                                @if(!empty($conf['text']))
                                    <div>
                                                        <span
                                                            class="block text-gray-400 uppercase font-bold text-[10px] mb-1">Gravurtext</span>
                                        <div
                                            class="font-serif italic text-gray-800 bg-gray-50 px-2 py-1.5 rounded border border-gray-100">
                                            "{{ $conf['text'] }}"
                                        </div>
                                    </div>
                                @endif
                                ...
                                @if(!empty($conf['notes']))
                                    <div>
                                                        <span
                                                            class="block text-gray-400 uppercase font-bold text-[10px] mb-1">Kunden-Anmerkung</span>
                                        <div
                                            class="text-gray-700 bg-yellow-50 px-2 py-1.5 rounded border border-yellow-100">
                                            {{ $conf['notes'] }}
                                        </div>
                                    </div>
                                @endif
                                ...
                                @if(count($files) > 0)
                                    <div class="col-span-1 xl:col-span-2">
                                                        <span
                                                            class="block text-gray-400 uppercase font-bold text-[10px] mb-1">Hochgeladene Dateien ({{ count($files) }})</span>
                                            ...
                                            @foreach($files as $file)
                                                <a href="{{ asset('storage/'.$file) }}" target="_blank"
                                                   ...
                                                   class="flex items-center gap-2 bg-white border border-gray-300 rounded px-3 py-1.5 hover:bg-gray-50 hover:border-primary hover:text-primary transition group/btn">
                                                    <svg
                                                        class="w-4 h-4 text-gray-500 group-hover/btn:text-primary"
        ...
        <div class="bg-gray-50 p-6 rounded-xl border border-gray-200">
            <h4 class="text-xs font-bold uppercase text-gray-500 mb-4 border-b border-gray-200 pb-2">
                Abrechnung</h4>
            <div class="space-y-3 text-sm">
                @php $originalSum = $this->selectedOrder->subtotal_price + ($this->selectedOrder->volume_discount ?? 0); @endphp
                <div class="flex justify-between text-gray-600"><span>Warenwert</span><span>{{ number_format($originalSum / 100, 2, ',', '.') }} €</span>
                </div>
                @if(isset($this->selectedOrder->volume_discount) && $this->selectedOrder->volume_discount > 0)
                    <div class="flex justify-between text-green-600"><span>Mengenrabatt</span><span>-{{ number_format($this->selectedOrder->volume_discount / 100, 2, ',', '.') }} €</span>
                    </div>
                @endif
                @if(isset($this->selectedOrder->discount_amount) && $this->selectedOrder->discount_amount > 0)
                    <div class="flex justify-between text-green-600"><span>Gutschein ({{ $this->selectedOrder->coupon_code }})</span><span>-{{ number_format($this->selectedOrder->discount_amount / 100, 2, ',', '.') }} €</span>
                    </div>
                @endif
                <div class="flex justify-between text-gray-600">
                    <span>Versand</span><span>{{ $this->selectedOrder->shipping_price > 0 ? number_format($this->selectedOrder->shipping_price / 100, 2, ',', '.') . ' €' : 'Kostenlos' }}</span>
                </div>
                <div class="pt-3 mt-1 border-t border-gray-200 flex justify-between items-end"><span
                        class="font-bold text-gray-900">Gesamtsumme</span><span
                        class="text-xl font-bold text-primary">{{ number_format($this->selectedOrder->total_price / 100, 2, ',', '.') }} €</span>
                </div>
            </div>
        </div>
