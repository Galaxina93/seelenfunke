<div style="--theme-color: {{ $this->themeColorHex }}; --theme-color-5: {{ $this->themeColorHex }}0D; --theme-color-10: {{ $this->themeColorHex }}1A; --theme-color-15: {{ $this->themeColorHex }}26; --theme-color-20: {{ $this->themeColorHex }}33; --theme-color-30: {{ $this->themeColorHex }}4D; --theme-color-40: {{ $this->themeColorHex }}66; --theme-color-50: {{ $this->themeColorHex }}80; --theme-color-70: {{ $this->themeColorHex }}B3;" class="min-h-screen bg-transparent p-4 lg:p-6 font-sans antialiased text-gray-300">
    ...
    @if($selectedQuoteId)
        @php
            $detailQuote = \App\Models\Order\Quote\QuoteRequest::with('items')->find($selectedQuoteId);
        @endphp
            ...
            <div class="max-w-6xl mx-auto animate-fade-in-up">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="bg-gray-900/80 backdrop-blur-xl rounded-[2.5rem] shadow-2xl border border-gray-800 overflow-hidden">
                            <div class="bg-gray-950 px-6 sm:px-8 py-5 border-b border-gray-800 flex items-center justify-between shadow-inner">
                                <h2 class="text-sm sm:text-base font-serif font-bold text-white tracking-wide">
                                    Positionen der Anfrage <span class="text-[var(--theme-color)] ml-1">#{{ $detailQuote->quote_number }}</span>
                                </h2>
                            </div>
                            ...
                            <div class="bg-gray-950 px-6 sm:px-8 py-6 border-t border-gray-800 shadow-inner">
                                <div class="flex flex-col gap-2 max-w-xs ml-auto">
                                    ...
                                    @endif
                                    <div class="flex justify-between items-end text-lg sm:text-xl font-serif font-black text-[var(--theme-color)] mt-4 pt-4 border-t border-gray-800">
                        ...
                        @if($detailQuote->status === 'open')
                            <div class="bg-gradient-to-br from-gray-900 to-gray-950 backdrop-blur-xl rounded-[2.5rem] shadow-[0_0_30px_rgba(0,0,0,0.5)] border border-[var(--theme-color-20)] p-6 sm:p-8 relative overflow-hidden group">
                                <div class="absolute top-0 right-0 w-64 h-64 bg-[var(--theme-color-5)] rounded-full blur-[80px] -translate-y-1/2 translate-x-1/3 pointer-events-none group-hover:bg-[var(--theme-color-10)] transition-colors duration-700"></div>
                        ...
                        <div class="bg-gray-900/80 backdrop-blur-md rounded-[2.5rem] shadow-2xl border border-gray-800 p-6 sm:p-8">
                            ...
                            <div class="space-y-4 text-sm font-medium text-gray-400">
                                ...
                                <div>
                                    ...
                                    <a href="mailto:{{ $detailQuote->email }}" class="text-[var(--theme-color)] hover:text-white transition-colors">{{ $detailQuote->email }}</a>
                                </div>
        ...
        @else
            <div class="max-w-3xl mx-auto text-center py-28 bg-gray-900/80 backdrop-blur-md rounded-[3rem] border border-gray-800 shadow-2xl">
                ...
                <button wire:click="closeDetail" class="text-[var(--theme-color)] font-black hover:text-white transition-colors uppercase tracking-widest text-[10px] border-b border-[var(--theme-color-30)] pb-0.5 hover:border-white">
    ...
    @else
        ...
        @php
            $stats = [
                'total' => \App\Models\Order\Quote\QuoteRequest::count(),
                'open' => \App\Models\Order\Quote\QuoteRequest::where('status', 'open')->count(),
                'converted' => \App\Models\Order\Quote\QuoteRequest::where('status', 'converted')->count(),
            ];
                    ...
                    @forelse($quotes as $quote)
                        <tr class="hover:bg-gray-800/30 transition-colors duration-300 group cursor-pointer"
                            ...
                            <td class="px-6 py-5 align-middle">
                                <span class="font-mono font-bold text-gray-300 group-hover:text-[var(--theme-color)] transition-colors text-base tracking-wide">{{ $quote->quote_number }}</span>
                            </td>
                            ...
                            <td class="px-6 py-5 text-right align-middle">
                                <button class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-gray-950 border border-gray-800 text-gray-400 group-hover:bg-[var(--theme-color)] group-hover:border-[var(--theme-color)] group-hover:text-gray-900 transition-all duration-300 transform group-hover:-translate-y-0.5 shadow-inner">
                ...
                <div wire:click="selectQuote('{{ $quote->id }}')"
                    ...
                    <div class="flex justify-between items-end">
                        ...
                        <div class="text-right flex-shrink-0">
                            <div class="font-serif font-bold text-[var(--theme-color)] text-xl whitespace-nowrap">{{ number_format($quote->gross_total / 100, 2, ',', '.') }} €</div>
                        </div>
                    </div>
                </div>
            @empty
