            ...
            <tr class="transition-all duration-300 group text-sm border-b border-gray-800 last:border-b-0 {{ $rowClass }}">
                ...
                <td class="px-6 py-5 whitespace-nowrap align-middle">
                    <div class="flex flex-col">
                        <span class="font-mono font-bold text-gray-300 group-hover:text-[var(--theme-color)] transition-colors cursor-pointer text-base tracking-wide"
                ...
                {{-- SPALTE 4: SUMME --}}
                <td class="px-6 py-5 text-right font-serif font-bold text-[var(--theme-color)] text-lg align-middle whitespace-nowrap">
                    ...
                    <div class="flex flex-col items-center gap-2">
                        <div class="relative inline-block w-40" wire:click.stop>
                            <select
                                ...
                                class="appearance-none w-full text-[10px] font-black uppercase tracking-widest rounded-xl border border-transparent py-2 pl-4 pr-8 cursor-pointer focus:ring-2 focus:ring-[var(--theme-color-20)] transition-all outline-none {{ $statusColors[$order->status] ?? 'bg-gray-800 text-gray-400' }}"
                            >
                ...
                <td class="px-6 py-5 text-right align-middle">
                    <button wire:click="openDetail('{{ $order->id }}')"
                            class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-gray-950 border border-gray-800 text-gray-400 hover:bg-[var(--theme-color)] hover:border-[var(--theme-color)] hover:text-gray-900 transition-all duration-300 transform hover:-translate-y-0.5 shadow-inner">
                ...
                <div wire:click="openDetail('{{ $order->id }}')"
                        ...
                        <div class="flex flex-col gap-1.5">
                            <span class="font-mono font-bold text-gray-300 text-base tracking-wide flex items-center gap-2">
                                ...
                                @if($order->shipping_address && serialize($order->billing_address) !== serialize($order->shipping_address))
                                    <svg class="w-3.5 h-3.5 text-[var(--theme-color)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" title="Abweichende Lieferadresse"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/></svg>
                                @endif
                            </span>
                    ...
                    <div class="flex justify-between items-end">
                        ...
                        <div class="text-right flex flex-col items-end">
                            <span class="font-serif font-bold text-xl text-[var(--theme-color)] whitespace-nowrap">
                                {{ number_format($order->total_price / 100, 2, ',', '.') }} €
                            </span>
                            @php
                                $totalItems = $order->items->count();
