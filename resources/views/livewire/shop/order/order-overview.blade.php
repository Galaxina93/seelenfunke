<div style="--theme-color: {{ $this->themeColorHex }}; --theme-color-5: {{ $this->themeColorHex }}0D; --theme-color-10: {{ $this->themeColorHex }}1A; --theme-color-15: {{ $this->themeColorHex }}26; --theme-color-20: {{ $this->themeColorHex }}33; --theme-color-30: {{ $this->themeColorHex }}4D; --theme-color-40: {{ $this->themeColorHex }}66; --theme-color-50: {{ $this->themeColorHex }}80; --theme-color-70: {{ $this->themeColorHex }}B3;" class="min-h-screen bg-transparent p-4 md:p-8 font-sans antialiased text-gray-300">
    ...
    @if(!$selectedOrderId)
            ...
            <div class="relative z-10 flex flex-col md:flex-row gap-8 sm:gap-12 items-center md:items-start">
                ...
                <div class="flex-1 text-center md:text-left pt-2 w-full">
                    <h2 class="font-serif font-bold text-3xl sm:text-4xl text-white mb-3 tracking-tight">
                        ...
                        @elseif($mood === 'chill')
                            Alles erledigt, {{ auth()->user()->first_name }}! <span class="text-[var(--theme-color)] drop-shadow-[0_0_10px_currentColor]">✨</span>
                        @else
                    ...
                    @if($hasPrio)
                        ...
                        <div class="flex flex-col xl:flex-row gap-5 items-stretch">
                            ...
                            <div wire:click="openDetail('{{ $prio->id }}')"
                                ...
                                {{-- Action Arrow --}}
                                <div class="w-10 h-10 rounded-full bg-gray-900 border border-gray-800 flex items-center justify-center text-gray-500 group-hover/card:bg-[var(--theme-color)] group-hover/card:text-gray-900 group-hover/card:border-[var(--theme-color)] transition-all duration-300 sm:ml-auto self-center shrink-0 shadow-inner">
        ...
        <div class="flex flex-col lg:flex-row justify-between items-center gap-4 mb-8 bg-gray-900/80 backdrop-blur-md p-2.5 sm:p-3 rounded-[2rem] shadow-2xl border border-gray-800 w-full">
            ...
            <div class="flex items-center gap-2 sm:gap-3 w-full lg:w-auto overflow-x-auto no-scrollbar pb-1 lg:pb-0 px-1">
                <select wire:model.live="statusFilter" class="px-4 py-3 bg-gray-950 border border-gray-800 rounded-[1.5rem] text-[10px] sm:text-xs font-black uppercase tracking-widest text-gray-400 focus:bg-black focus:ring-2 focus:ring-[var(--theme-color)] focus:border-[var(--theme-color)] cursor-pointer hover:bg-gray-800 hover:text-white transition-all outline-none shadow-inner">
        ...
        @if($orders->isNotEmpty())
            <div class="mb-12 animate-fade-in-up w-full">
                ...
                <div class="bg-gray-900/80 backdrop-blur-md rounded-[2rem] shadow-2xl border border-gray-800 overflow-hidden w-full">
                    @include('livewire.shop.order.order-overview-partials.table', [
                        'orders' => $orders,
                        'sortField' => $sortField,
        ...
        @else
            ...
            <div class="text-center py-28 bg-gray-900/80 backdrop-blur-md rounded-[3rem] border border-gray-800 shadow-2xl w-full">
                ...
                <button wire:click="resetFilters" class="text-[var(--theme-color)] font-black hover:text-white transition-colors uppercase tracking-widest text-[10px] border-b border-[var(--theme-color-30)] pb-0.5 hover:border-white">
                    Filter zurücksetzen
                </button>
            </div>
        @endif
