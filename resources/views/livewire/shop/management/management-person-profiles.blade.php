        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-gray-900/80 backdrop-blur-md p-6 sm:p-10 rounded-[2.5rem] shadow-2xl border border-gray-800 relative overflow-hidden mb-8">
            ...
            <div class="relative z-10">
                <h1 class="text-3xl sm:text-4xl font-serif font-bold text-white tracking-tight flex items-center gap-4">
                    <span class="text-[var(--theme-color)]"><x-heroicon-o-users class="w-10 h-10" /></span>
                    Kontakte
                </h1>
        ...
        <div class="flex flex-col lg:flex-row h-auto lg:h-[calc(100vh-18rem)] lg:min-h-[700px] bg-gray-900/80 backdrop-blur-xl rounded-[2.5rem] shadow-2xl border border-gray-800 overflow-visible lg:overflow-hidden">
            ...
            <div class="w-full lg:w-1/3 xl:w-1/4 h-[50vh] lg:h-auto bg-gray-950/50 border-b lg:border-b-0 lg:border-r border-gray-800 flex flex-col shrink-0 z-10 shadow-inner">
                ...
                <div class="p-3 border-b border-gray-800 shrink-0 flex items-center justify-between gap-3">
                    ...
                    <button wire:click="createProfile" class="flex-1 py-2 bg-[var(--theme-color-10)] text-[var(--theme-color)] border border-[var(--theme-color-20)] rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-[var(--theme-color-20)] hover:border-[var(--theme-color-40)] transition-all flex items-center justify-center gap-2">
                    ...
                    <div class="flex-1 overflow-y-auto custom-scrollbar p-8 lg:p-12">
                        ...
                        <div class="space-y-8">
                            ...
                            <div class="bg-gray-900/50 border border-gray-800 rounded-3xl p-6 flex flex-col items-center justify-center relative group overflow-hidden">
                                <h4 class="text-xs font-black text-[var(--theme-color)] mb-4 uppercase tracking-widest w-full text-left flex items-center gap-2">
                                    <x-heroicon-o-photo class="w-4 h-4" /> Profilbild
                                </h4>

                                <label for="avatar_upload" class="cursor-pointer flex flex-col items-center justify-center w-32 h-32 rounded-full border-2 border-dashed border-gray-700 bg-gray-950/80 hover:border-[var(--theme-color-50)] hover:bg-[var(--theme-color-5)] transition-all relative overflow-hidden">
                                    @if ($avatar_upload)
                                    ...
                                    @else
                                        <x-heroicon-o-cloud-arrow-up class="w-8 h-8 text-gray-500 mb-2 group-hover:text-[var(--theme-color)] transition-colors" />
                                        <span class="text-[10px] uppercase font-bold text-gray-500 group-hover:text-[var(--theme-color)] transition-colors">Drag & Drop</span>
                                    @endif
                                ...
                                <div wire:loading wire:target="avatar_upload" class="text-xs text-[var(--theme-color)] mt-2 flex items-center gap-2 font-bold bg-[var(--theme-color-10)] px-3 py-1 rounded-full border border-[var(--theme-color-20)]">
                            ...
                            <!-- Stammdaten -->
                            <div>
                                <h4 class="text-xs font-black text-[var(--theme-color)] mb-4 uppercase tracking-widest flex items-center gap-2">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="text" wire:model="editForm.first_name" class="w-full bg-gray-950 border border-gray-800 rounded-lg text-white px-4 py-2 focus:ring-[var(--theme-color)] focus:border-[var(--theme-color)] outline-none transition-all">
                                    ...
                                    <div>
                                        ...
                                        <input type="text" wire:model="editForm.last_name" class="w-full bg-gray-950 border border-gray-800 rounded-lg text-white px-4 py-2 focus:ring-[var(--theme-color)] focus:border-[var(--theme-color)] outline-none transition-all">
                                    </div>
                                    <div>
                                        <label class="block text-[10px] uppercase tracking-widest text-gray-500 mb-1">Rufname / Alias</label>
                                        <input type="text" wire:model="editForm.nickname" placeholder="z.B. Mama, Schatz" class="w-full bg-gray-950 border border-gray-800 rounded-lg text-white px-4 py-2 focus:ring-[var(--theme-color)] focus:border-[var(--theme-color)] outline-none transition-all">
                                    </div>
                                    <div>
                                        <label class="block text-[10px] uppercase tracking-widest text-gray-500 mb-1">Beziehung</label>
                                        <input type="text" wire:model="editForm.relation_type" placeholder="z.B. Bruder, Freundin" class="w-full bg-gray-950 border border-gray-800 rounded-lg text-white px-4 py-2 focus:ring-[var(--theme-color)] focus:border-[var(--theme-color)] outline-none transition-all">
                                    </div>
                                    <div>
                                        <label class="block text-[10px] uppercase tracking-widest text-gray-500 mb-1">Geburtstag</label>
                                        <input type="date" wire:model="editForm.birthday" class="w-full bg-gray-950 border border-gray-800 rounded-lg text-white px-4 py-2 focus:ring-[var(--theme-color)] focus:border-[var(--theme-color)] outline-none transition-all [color-scheme:dark]">
                                    </div>
                                </div>
                            </div>
                ...
                @elseif($activeProfile)
                    ...
                    <div class="flex-1 overflow-y-auto custom-scrollbar p-8 lg:p-12 space-y-8 bg-gray-950/20">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            <a href="tel:{{ preg_replace('/[^0-9+]/', '', $activeProfile->phone) }}" class="bg-gray-900 border border-gray-800 rounded-2xl p-4 flex items-center gap-4 hover:border-[var(--theme-color-50)] hover:bg-gray-800 transition-colors cursor-pointer group">
                                <div class="w-10 h-10 rounded-full bg-gray-800 text-gray-400 flex items-center justify-center group-hover:text-[var(--theme-color)] group-hover:bg-[var(--theme-color-10)] transition-colors shadow-inner"><x-heroicon-o-phone class="w-5 h-5 group-hover:scale-110 transition-transform" /></div>
                                <div><p class="text-[10px] text-gray-400 group-hover:text-[var(--theme-color)] uppercase tracking-widest font-bold transition-colors">Telefon</p><p class="text-gray-200 font-medium">{{ $activeProfile->phone }}</p></div>
                            </a>
                            @endif
                ...
                @else
                    ...
                    <div class="flex-1 flex flex-col items-center justify-center text-gray-500 p-10 text-center">
                        <div class="w-24 h-24 bg-gray-950 border border-gray-800 rounded-full flex items-center justify-center shadow-inner mb-6 relative group">
                            <div class="absolute inset-0 bg-[var(--theme-color-20)] rounded-full blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-1000"></div>
                            <x-heroicon-o-user-group class="w-10 h-10 text-gray-700 relative z-10 transition-colors duration-1000 group-hover:text-[var(--theme-color)]" />
                        </div>
