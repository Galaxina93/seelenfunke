<div style="--theme-color: {{ $this->themeColorHex }}; --theme-color-5: {{ $this->themeColorHex }}0D; --theme-color-10: {{ $this->themeColorHex }}1A; --theme-color-15: {{ $this->themeColorHex }}26; --theme-color-20: {{ $this->themeColorHex }}33; --theme-color-30: {{ $this->themeColorHex }}4D; --theme-color-40: {{ $this->themeColorHex }}66; --theme-color-50: {{ $this->themeColorHex }}80; --theme-color-70: {{ $this->themeColorHex }}B3;" class="min-h-screen bg-slate-50/50 flex items-center justify-center px-4 py-12 sm:px-6 lg:px-8 font-sans">
    <div class="max-w-md w-full bg-white p-8 sm:p-10 rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-100 relative overflow-hidden transition-all duration-500 hover:shadow-2xl">
        ...
        <div class="absolute top-0 right-0 w-64 h-64 bg-[var(--theme-color-5)] rounded-full blur-3xl -z-10 translate-x-1/3 -translate-y-1/3"></div>
        ...
        <div class="flex justify-center mb-8 bg-slate-50 p-1 rounded-2xl inline-flex w-full">
            <button wire:click="$set('activeTab', 'subscribe')"
                    class="flex-1 py-2.5 px-4 rounded-xl text-sm font-bold transition-all duration-300 {{ $activeTab === 'subscribe' ? 'bg-white text-[var(--theme-color)] shadow-sm ring-1 ring-slate-100' : 'text-slate-400 hover:text-slate-600' }}">
                Anmelden
            </button>
        ...
        @if($activeTab === 'subscribe' && !$successMessage)
            <form class="space-y-5 animate-fade-in" wire:submit.prevent="subscribe" wire:key="form-subscribe">
                <div class="group">
                    <div class="relative">
                        <input id="email-address" wire:model="email" type="email" required
                               class="block w-full px-5 py-4 bg-slate-50 border-0 rounded-2xl text-slate-800 placeholder-slate-400 focus:ring-2 focus:ring-[var(--theme-color-20)] focus:bg-white transition-all shadow-inner text-sm font-medium"
                               placeholder="Deine E-Mail Adresse">
                        <div class="absolute inset-y-0 right-4 flex items-center pointer-events-none text-[var(--theme-color-50)]">
                ...
                <div class="flex items-start gap-3 px-1">
                    <div class="flex items-center h-5 mt-0.5">
                        <input id="privacy" wire:model.live="privacy_accepted" type="checkbox"
                               class="w-4 h-4 text-[var(--theme-color)] bg-slate-100 border-slate-300 rounded focus:ring-[var(--theme-color)] focus:ring-offset-0 cursor-pointer transition-colors">
                    </div>
                    <div class="text-xs text-slate-500 leading-relaxed">
                        <label for="privacy" class="font-medium text-slate-700 cursor-pointer select-none hover:text-[var(--theme-color)] transition-colors">Datenschutz akzeptieren</label>
                        <p class="mt-1">Details in der <a href="{{ route('datenschutz') }}" class="text-[var(--theme-color)] font-bold hover:underline underline-offset-2">Datenschutzerklärung</a>.</p>
                    </div>
                </div>
                ...
                class="group relative w-full flex justify-center py-4 px-6 text-sm font-black uppercase tracking-wider rounded-2xl transition-all duration-300
                               disabled:bg-slate-100 disabled:text-slate-400 enabled:bg-[var(--theme-color)] enabled:text-white enabled:shadow-lg enabled:hover:-translate-y-1 overflow-hidden">
            ...
            <div class="mt-4 animate-fade-in" x-data="{ state: 'normal' }" wire:key="form-unsubscribe">
                ...
                <div class="relative flex flex-col items-center justify-center h-48 mb-6">
                    ...
                    <div class="relative w-32 h-32 transition-transform duration-500"
                        ...
                        <img src="{{ asset('shop/projekt/funki/funki_cry.webp') }}"
                        ...
                        <img src="{{ asset('shop/projekt/funki/funki_cry_more.webp') }}"
                             class="absolute inset-0 w-full h-full object-contain transition-opacity duration-300"
                             :class="state === 'focus' ? 'opacity-100' : 'opacity-0'">

                        <img src="{{ asset('shop/projekt/funki/funki_gave_up.webp') }}"
                             class="absolute inset-0 w-full h-full object-contain transition-opacity duration-300"
