<div class="w-full min-h-screen bg-gray-950 relative" x-data="goldDust()">
    @include('livewire.customer.partials.scripts')

    {{-- HINTERGRUND FUNKEN / METEORITEN --}}
    <canvas id="gold-dust-canvas" class="fixed inset-0 z-[10] pointer-events-none" wire:ignore></canvas>
    ...
    @if(!$hasOptedIn)
        <div class="w-full min-h-screen overflow-y-auto bg-gray-950 pb-24 relative scroll-smooth z-20">

            {{-- STANDARD HEADER (Wenn Spielmodus AUS ist) --}}
            ...
            <div x-data="optInScreen()" @mousemove="handleMouse($event)" @mouseleave="resetMouse()" class="max-w-[1600px] mx-auto bg-gray-900 rounded-none shadow-[0_20px_50px_rgba(0,0,0,0.5)] border-y border-gray-800 relative p-8 md:p-20 flex flex-col md:flex-row items-center gap-16 overflow-hidden transition-all duration-700 mt-12">
                ...
                <div class="relative z-10 flex-1">
                    ...
                    <ul class="space-y-6 mb-12">
                        <li class="flex items-center gap-5 text-gray-400 group cursor-default hover:scale-[1.02] hover:translate-x-2 transition-all duration-300 relative" @mouseenter="window.spawnSparks ? window.spawnSparks($event) : null">
    ...
    @if($hasOptedIn)
        <div x-data="funkiHub('{{ $modelPath }}', '{{ $imagePath }}')" x-init="initShop()" @funki-level-up.window="handleLevelUp($event.detail[0])" class="bg-gray-900 shadow-2xl overflow-hidden border-t border-gray-800 relative animate-fade-in-up min-h-screen z-20">

            <div x-show="startupFlash" x-transition:leave="transition ease-out duration-1000 delay-300" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="absolute inset-0 z-50 bg-white pointer-events-none" style="display: none;"></div>

            {{-- EP BAR / HEADER ICONS RESTORE --}}
            ...
            <section id="orders-section" class="bg-gray-900 text-white py-32 px-6 md:px-12 border-t border-gray-800 relative z-20">
                <div class="max-w-[1600px] mx-auto">
                    <div class="flex flex-col md:flex-row md:items-center gap-8 mb-16">
                        ...
                        <div>
                            <h2 class="text-4xl md:text-5xl font-serif font-bold mb-2 tracking-tight">Deine Bestellungen</h2>
                            <p class="text-gray-400 font-black uppercase tracking-widest text-xs">Die Historie deiner persönlichen Schätze</p>
                        </div>
                    </div>
                    <div class="bg-transparent overflow-hidden">
                        @include('livewire.customer.partials.orders_section')
            ...
            {{-- MODAL: 3D Viewer + Sticky Info + Evolutions-Timeline --}}
            @include('livewire.customer.partials.evolution_timeline')

            {{-- MODAL: Titelverwaltung --}}
