...
<div class="w-full" x-data="{ showLightbox: false, lightboxImage: '' }" @keydown.escape.window="showLightbox = false">
    ...
    @if($step > 0)
        <div class="relative bg-white rounded-xl shadow-2xl overflow-hidden border border-gray-100 my-10" id="calculator-anchor">

            {{-- Das Overlay mit Spinner und Blur-Effekt (Triggert bei submit) --}}
            <div wire:loading wire:target="submit"
                 class="fixed inset-0 lg:absolute z-[100] flex flex-col items-center justify-center bg-white/70 backdrop-blur-md rounded-xl transition-all">
                <div class="flex flex-col items-center bg-white p-8 rounded-3xl shadow-2xl border border-gray-100">
                    ...
                    <h3 class="text-xl font-serif font-bold text-gray-900">Angebot wird erstellt</h3>
                    <p class="text-gray-500 text-sm mt-2">Wir bereiten Ihr PDF-Angebot vor und senden es Ihnen zu...</p>
                </div>
            </div>

            {{-- Der eigentliche Inhalt, der bei Belastung unscharf/blass wird --}}
            <div wire:loading.class="opacity-30 blur-[2px] pointer-events-none" wire:target="submit">
