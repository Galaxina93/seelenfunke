<div x-data="{
     ...
     class="flex flex-nowrap items-center justify-start xl:justify-end gap-2 md:gap-3 bg-gray-900/80 backdrop-blur-xl p-2 rounded-2xl border border-gray-800 shadow-inner overflow-x-auto no-scrollbar w-fit max-w-full z-[999] snap-x snap-mandatory touch-pan-x"
        ...
        <div x-show="showProfileModal" style="display: none;" class="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-10 text-left">
            ...
            <div class="relative w-full max-w-5xl h-full max-h-[90vh] flex flex-col bg-gray-900/90 backdrop-blur-xl rounded-[2.5rem] shadow-[0_0_50px_rgba(0,0,0,0.5)] border border-gray-800 overflow-hidden"
                ...
                <div class="flex-1 overflow-y-auto p-6 sm:p-8 custom-scrollbar relative bg-transparent">
                    ...
                    {{-- TAB: PROFIL --}}
                    @include('livewire.auth.partials.tab_profile')

                    {{-- TAB: PASSWORT --}}
                    @include('livewire.auth.partials.tab_password')

                    {{-- TAB: 2-FA SCHUTZ --}}
                    @include('livewire.auth.partials.tab_2fa')

                    {{-- TAB: GERÄTE & SITZUNGEN --}}
                    @include('livewire.auth.partials.tab_devices')

                    {{-- TAB: KONTO LÖSCHEN --}}
                    @include('livewire.auth.partials.tab_delete_account')
