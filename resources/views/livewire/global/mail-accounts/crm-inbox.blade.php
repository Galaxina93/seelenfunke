                ...
                <div class="bg-gray-900 border border-gray-800 rounded-2xl p-6 w-full max-w-md shadow-2xl" @keydown.escape.window="$wire.set('showNewFolderModal', false)">
                    ...
                    <form wire:submit.prevent="createFolder">
                        <div class="mb-4">
                            ...
                            <input type="text" wire:model="newFolderName" class="w-full bg-gray-950 border border-gray-700 text-white rounded-lg focus:ring-primary focus:border-primary px-4 py-2" placeholder="z.B. Rechnungen, Projekte..." required autofocus>
                        ...
                        <div class="flex justify-end gap-3 mt-6">
                            ...
                            <button type="submit" class="bg-primary hover:bg-primary/80 text-black px-6 py-2 text-sm font-bold rounded-lg transition-colors">Erstellen</button>
                        </div>
                    </form>
                </div>
            </div>
                ...
                <div class="bg-gray-900 border border-gray-800 rounded-2xl p-6 w-full max-w-md shadow-2xl" @keydown.escape.window="$wire.set('showRoutingModal', false)">
                    ...
                    <form wire:submit.prevent="saveRoutingRule">
                        <div class="mb-4">
                            ...
                            <select wire:model="routingTargetFolder" class="w-full bg-gray-950 border border-gray-700 text-white rounded-lg focus:ring-primary focus:border-primary px-4 py-2" required>
                        ...
                        <div class="flex justify-end gap-3 mt-6">
                            ...
                            <button type="submit" class="bg-primary hover:bg-primary/80 text-black px-6 py-2 text-sm font-bold rounded-lg transition-colors">Regel aktivieren</button>
                        </div>
                    </form>
                </div>
            </div>
                ...
                <div class="bg-gray-900 border border-gray-800 rounded-2xl p-0 w-full max-w-4xl shadow-2xl h-[80vh] flex flex-col" @keydown.escape.window="$wire.set('showComposeModal', false)">
                    ...
                    <form wire:submit.prevent="sendMail" class="flex flex-col flex-1 overflow-hidden">
                        ...
                        <div class="p-4 border-t border-gray-800 bg-gray-950 rounded-b-2xl flex justify-end gap-3 shrink-0">
                            ...
                            <button type="submit" class="bg-primary hover:bg-primary/80 text-black px-8 py-2 text-sm font-bold rounded-lg transition-colors flex items-center gap-2">
                    ...
                    <div x-data="{ open: {{ $isAccActive ? 'true' : 'false' }} }" class="space-y-1">
                        ...
                        <div class="w-full flex items-center justify-between px-2 py-1.5 rounded-lg text-sm font-bold transition-colors {{ $isAccActive ? 'text-white bg-gray-800/50' : 'text-gray-400 hover:text-gray-200 hover:bg-gray-800/30' }}">
                            ...
                            <button @click="open = !open; $wire.selectAccount({{ $acc->id }})" class="flex items-center gap-2 truncate flex-1 text-left">
                                <x-heroicon-s-chevron-right class="w-4 h-4 transition-transform duration-200 shrink-0" x-bind:class="open ? 'rotate-90 text-primary' : ''" />
                                <x-heroicon-o-at-symbol class="w-4 h-4 shrink-0 {{ $isAccActive ? 'text-primary' : '' }}"/>
                                <span class="truncate">{{ $acc->name }}</span>
                            </button>
                            
                            <div class="flex items-center gap-1 shrink-0">
                                @if($accTotalUnread > 0)
                                    <span class="bg-primary text-black text-[10px] font-bold px-2 py-0.5 rounded-full">{{ $accTotalUnread }}</span>
                                @endif
                                ...
                                @foreach($accFolders as $key => $label)
                                    ...
                                    <div class="relative group"
                                        ...
                                        <button wire:click.stop="selectAccountAndFolder({{ $acc->id }}, '{{ $key }}')" 
                                                :class="isDragOver ? 'ring-1 ring-primary bg-primary/20' : ''"
                                                class="w-full flex items-center justify-between px-3 py-2 rounded-lg text-xs font-medium transition-colors {{ $isActiveFolder ? 'bg-primary/10 text-primary' : 'text-gray-400 hover:bg-gray-800 hover:text-white' }}">
                                            <div class="flex items-center gap-3 truncate">
                                                <x-dynamic-component :component="'heroicon-o-'.$icon" class="w-4 h-4 shrink-0 {{ $isActiveFolder ? 'text-primary' : 'text-gray-500' }}" />
    ...
    <div class="absolute inset-0 z-20 lg:relative lg:z-auto lg:flex-1 overflow-hidden flex transition-transform duration-300 pointer-events-none lg:pointer-events-auto"
        ...
        <div class="px-4 py-3 border-b border-gray-800 bg-gray-950/50 flex flex-col gap-2">
            <div class="relative">
                ...
                <input type="text" wire:model.live.debounce.300ms="searchQuery" placeholder="Suchen..." class="w-full bg-gray-900 border border-gray-800 text-white text-sm rounded-lg pl-9 pr-3 py-2 focus:ring-1 focus:ring-primary focus:border-primary placeholder-gray-600 transition-colors">
            </div>
            <select wire:model.live="filterMode" class="w-full bg-gray-900 border border-gray-800 text-gray-300 text-xs rounded-lg px-3 py-1.5 focus:ring-1 focus:ring-primary focus:border-primary">
        ...
        <div class="flex-1 overflow-y-auto">
            @forelse($messages as $msg)
                ...
                <div x-data="{ contextMenuOpen: false }" 
                    ...
                    <button wire:click="selectMessage({{ $msg->id }})" class="w-full text-left p-4 hover:bg-gray-800 transition-colors {{ $isMsgActive ? 'bg-gray-800 border-l-2 border-primary' : '' }} {{ !$msg->is_read ? 'bg-gray-900/40' : '' }}">
            ...
            <div class="p-4 sm:p-8 max-w-5xl mx-auto">
                ...
                <form wire:submit.prevent="saveAccount" class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div class="lg:col-span-2 space-y-4">
                        <h4 class="text-primary font-bold text-sm tracking-widest uppercase border-b border-gray-800 pb-2">Kontodetails</h4>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs text-gray-400 mb-1">Anzeigename</label>
                                <input type="text" wire:model="account_name" class="w-full bg-gray-950 border border-gray-800 text-white rounded-lg px-3 py-2 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" required placeholder="z.B. Zentrale Inbox" />
                            </div>
                            <div>
                                <label class="block text-xs text-gray-400 mb-1">E-Mail Adresse</label>
                                <input type="email" wire:model.live.debounce.500ms="email" class="w-full bg-gray-950 border border-gray-800 text-white rounded-lg px-3 py-2 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" required placeholder="hallo@mein-seelenfunke.de" />
                            </div>
                            <div class="md:col-span-2" x-data="{ showPassword: false }">
                                ...
                                <div class="relative">
                                    <input :type="showPassword ? 'text' : 'password'" wire:model="password" class="w-full bg-gray-950 border border-gray-800 text-white rounded-lg pl-3 pr-10 py-2 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" {{ $editAccountId ? '' : 'required' }} />
                                    <button type="button" @click="showPassword = !showPassword" class="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-500 hover:text-primary transition-colors">
                    ...
                    {{-- IMAP Settings --}}
                    <div class="space-y-4">
                        <h4 class="text-primary font-bold text-sm tracking-widest uppercase border-b border-gray-800 pb-2">Eingangsserver (IMAP)</h4>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs text-gray-400 mb-1">IMAP Host</label>
                                <input type="text" wire:model="imap_host" class="w-full bg-gray-950 border border-gray-800 text-white rounded-lg px-3 py-2 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" required />
                            </div>
                            <div>
                                <label class="block text-xs text-gray-400 mb-1">Benutzer (optional)</label>
                                <input type="text" wire:model="imap_username" class="w-full bg-gray-950 border border-gray-800 text-white rounded-lg px-3 py-2 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" />
                            </div>
                            <div>
                                <label class="block text-xs text-gray-400 mb-1">Port</label>
                                <input type="number" wire:model="imap_port" class="w-full bg-gray-950 border border-gray-800 text-white rounded-lg px-3 py-2 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" required />
                            </div>
                            <div>
                                <label class="block text-xs text-gray-400 mb-1">Verschlüsselung</label>
                                <select wire:model="imap_encryption" class="w-full bg-gray-950 border border-gray-800 text-white rounded-lg px-3 py-2 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary">
                    ...
                    {{-- SMTP Settings --}}
                    <div class="space-y-4">
                        <h4 class="text-primary font-bold text-sm tracking-widest uppercase border-b border-gray-800 pb-2">Ausgangsserver (SMTP)</h4>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs text-gray-400 mb-1">SMTP Host</label>
                                <input type="text" wire:model="smtp_host" class="w-full bg-gray-950 border border-gray-800 text-white rounded-lg px-3 py-2 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" required />
                            </div>
                            <div>
                                <label class="block text-xs text-gray-400 mb-1">Benutzer (optional)</label>
                                <input type="text" wire:model="smtp_username" class="w-full bg-gray-950 border border-gray-800 text-white rounded-lg px-3 py-2 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" />
                            </div>
                            <div>
                                <label class="block text-xs text-gray-400 mb-1">Port</label>
                                <input type="number" wire:model="smtp_port" class="w-full bg-gray-950 border border-gray-800 text-white rounded-lg px-3 py-2 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" required />
                            </div>
                            <div>
                                <label class="block text-xs text-gray-400 mb-1">Verschlüsselung</label>
                                <select wire:model="smtp_encryption" class="w-full bg-gray-950 border border-gray-800 text-white rounded-lg px-3 py-2 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary">
                                    <option value="ssl">SSL / TLS</option>
                                    <option value="tls">STARTTLS</option>
                                    <option value="">Keine</option>
                                </select>
                    ...
                    {{-- Signatur --}}
                    <div class="lg:col-span-2 space-y-4">
                        <h4 class="text-primary font-bold text-sm tracking-widest uppercase border-b border-gray-800 pb-2">Signatur</h4>
                        <div>
                            <textarea wire:model="signature" rows="4" class="w-full bg-gray-950 border border-gray-800 text-white rounded-lg px-3 py-2 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary font-mono text-xs"></textarea>
                        </div>
                    </div>

                    <div class="lg:col-span-2 pt-6 border-t border-gray-800 flex justify-end gap-3">
                        <button type="submit" class="px-6 py-2 bg-primary text-black hover:bg-primary/80 font-bold rounded-lg transition-colors">Konto speichern</button>
                    </div>
                </form>
            </div>
