    ...
    <div class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3 md:gap-4">
        @php
            $renderKpi = function($key, $title, $value, $subValue = null, $dotColor = 'bg-gray-600', $textColor = 'text-white', $bg = 'bg-gray-900/80 backdrop-blur-md', $border = 'border-gray-800') use ($stats, $infoTexts) {
                return "
                <div x-data='{show: false}' :class=\"show ? 'z-50' : 'z-10'\" class='relative $bg p-3.5 sm:p-5 rounded-xl sm:rounded-2xl shadow-2xl border $border transition-all hover:border-primary/50 flex flex-col justify-between min-h-[90px]'>
                    <div class='flex items-start sm:items-center justify-between mb-2 gap-2'>
                        ...
                        <div class='flex items-center shrink-0'>
                            ...
                            <div x-show='show' x-cloak x-transition.opacity.duration.200ms class='absolute w-48 sm:w-56 p-3 bg-gray-900 border border-gray-700 text-gray-300 text-[10px] sm:text-[11px] font-medium rounded-xl shadow-[0_20px_40px_rgba(0,0,0,0.8)] pointer-events-none tracking-wide whitespace-normal right-0 sm:right-auto sm:left-1/2 sm:-translate-x-1/2 top-full mt-3 z-[100]'>
                                <div class='absolute -top-1.5 right-2 sm:right-auto sm:left-1/2 sm:-translate-x-1/2 w-3 h-3 bg-gray-900 border-t border-l border-gray-700 transform rotate-45'></div>
                                <div class='relative z-10'>{$infoTexts[$key]}</div>
                            </div>
                        </div>
                    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3 sm:gap-4">
        @php
            $renderSmallKpi = function($key, $title, $value, $color) use ($infoTexts) {
                return "
                <div x-data='{show: false}' :class=\"show ? 'z-50' : 'z-10'\" class='relative bg-gray-950 px-3 py-3 md:px-5 md:py-4 rounded-xl sm:rounded-2xl border border-gray-800 shadow-inner flex flex-col'>
                    <div class='flex items-center justify-between mb-1.5 gap-2'>
                        ...
                        <div class='flex items-center shrink-0'>
                            ...
                            <div x-show='show' x-cloak x-transition.opacity.duration.200ms class='absolute w-48 p-3 bg-gray-900 border border-gray-700 text-gray-300 text-[9px] sm:text-[10px] rounded-xl shadow-[0_20px_40px_rgba(0,0,0,0.8)] pointer-events-none tracking-wide font-medium whitespace-normal right-0 sm:right-auto sm:left-1/2 sm:-translate-x-1/2 top-full mt-3 z-[100]'>
                                <div class='absolute -top-1.5 right-2 sm:right-auto sm:left-1/2 sm:-translate-x-1/2 w-3 h-3 bg-gray-900 border-t border-l border-gray-700 transform rotate-45'></div>
                                <div class='relative z-10'>{$infoTexts[$key]}</div>
                            </div>
                        </div>
