<div class="bg-gray-900/80 backdrop-blur-md rounded-[1.5rem] md:rounded-[2.5rem] shadow-2xl border border-gray-800 p-5 md:p-8 xl:col-span-3 flex flex-col w-full overflow-hidden relative group">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        ...
        <div class="flex flex-wrap gap-3">
            <div class="bg-gray-950 p-3 md:p-4 rounded-xl md:rounded-2xl border border-gray-800 shadow-inner flex flex-col items-center min-w-[100px]">
                ...
                <span class="text-xl md:text-2xl font-black text-white">{{ number_format($stats['frontend_visits_total'], 0, ',', '.') }}</span>
            </div>
            <div class="bg-gray-950 p-3 md:p-4 rounded-xl md:rounded-2xl border border-gray-800 shadow-inner flex flex-col items-center min-w-[100px]">
                <span class="text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 mb-1">Besucher</span>
                <span class="text-xl md:text-2xl font-black text-blue-400 drop-shadow-[0_0_10px_rgba(96,165,250,0.4)]">{{ number_format($stats['frontend_unique_total'], 0, ',', '.') }}</span>
            </div>

            @php
                $totalDevice = max(1, $stats['desktop_visits'] + $stats['mobile_visits']);
                $mobilePct = round(($stats['mobile_visits'] / $totalDevice) * 100);
            @endphp
    ...
    <div class="w-full mb-8">
        <div class="flex items-center gap-4 mb-4 justify-end">
            <span class="flex items-center gap-2 text-[9px] font-black text-white uppercase tracking-widest bg-gray-950 px-3 py-1.5 rounded-full border border-gray-800">
                <span class="w-2.5 h-2.5 rounded-full bg-blue-500 shadow-[0_0_8px_currentColor]"></span> Seitenaufrufe
            </span>
            <span class="flex items-center gap-2 text-[9px] font-black text-white uppercase tracking-widest bg-gray-950 px-3 py-1.5 rounded-full border border-gray-800">
                <span class="w-2.5 h-2.5 rounded-full bg-emerald-400 shadow-[0_0_8px_currentColor]"></span> Eindeutige Besucher
            </span>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8 pt-6 border-t border-gray-800">
        <div class="bg-gray-950/50 rounded-2xl p-5 border border-gray-800 shadow-inner">
            ...
            <div class="space-y-3">
                @forelse($stats['top_pages'] as $page)
                    @php
                        $pct = $stats['frontend_visits_total'] > 0 ? round(($page->count / $stats['frontend_visits_total']) * 100) : 0;
                    @endphp
        ...
        <div class="bg-gray-950/50 rounded-2xl p-5 border border-gray-800 shadow-inner">
            ...
            <div class="space-y-3">
                @forelse($stats['top_referrers'] as $host => $count)
                    <div class="flex items-center justify-between p-2 rounded-xl hover:bg-gray-800/50 transition-colors">
                        <div class="flex items-center gap-3">
                            <div class="w-6 h-6 rounded bg-gray-800 flex items-center justify-center text-[10px] font-bold text-gray-500 uppercase">
                                {{ substr($host, 0, 2) }}
                            </div>
