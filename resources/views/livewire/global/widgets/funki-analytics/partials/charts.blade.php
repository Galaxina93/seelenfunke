    ...
    <div class="lg:col-span-2 bg-gray-900/80 backdrop-blur-md rounded-[3rem] p-10 shadow-2xl border border-gray-800 relative overflow-hidden flex flex-col justify-center group transition-all hover:border-primary/40 hover:shadow-[0_0_40px_rgba(197,160,89,0.15)]">
        ...
        <div class="relative z-10 flex flex-col items-center text-center">
            ...
            @if(isset($stats['high_revenue_prod']) && $stats['high_revenue_prod'])
                <div class="text-3xl md:text-4xl font-serif font-bold text-white leading-tight max-w-lg mx-auto">{{ $stats['high_revenue_prod']['product_name'] ?? 'Unbekannt' }}</div>
                <div class="text-5xl font-black text-primary tracking-tighter mt-4 drop-shadow-[0_0_15px_rgba(197,160,89,0.4)]">{{ number_format($stats['high_revenue_prod']['total'] ?? 0, 2, ',', '.') }} €</div>
            @else
        ...
        <div class="relative z-10 flex flex-col items-center text-center">
            <h4 class="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] mb-3 flex items-center gap-2">
                <i class="solar-graph-down-bold-duotone text-red-400 text-lg"></i>
                Performance-Tief
            </h4>

            @if(isset($stats['low_revenue_prod']) && $stats['low_revenue_prod'])
                <div class="text-lg font-bold text-gray-300 max-w-md mx-auto">{{ $stats['low_revenue_prod']['product_name'] ?? 'Unbekannt' }}</div>
                <div class="text-2xl font-black text-red-400 tracking-tight mt-2 drop-shadow-[0_0_10px_rgba(248,113,113,0.3)]">{{ number_format($stats['low_revenue_prod']['total'] ?? 0, 2, ',', '.') }} €</div>
            @else
