<div class="relative font-sans">
    {{-- Etwas näher an den Rand gerückt auf mobilen Geräten --}}
    <div class="fixed z-[9999] flex gap-3 sm:gap-4 pointer-events-none bottom-4 right-4 sm:bottom-6 sm:right-6 flex-col items-end">
        ...
        <div x-data="{ isMaximized: false }"
            ...
            <div class="bg-slate-900 p-4 sm:p-5 flex items-center justify-between relative overflow-hidden shrink-0">
                ...
                <div class="flex items-center gap-3 sm:gap-4 relative z-10">
                    <div class="w-10 h-10 rounded-xl bg-white/10 p-1 backdrop-blur-xl border border-white/20">
                        <img src="{{ asset('images/projekt/funki/funki_selfie.png') }}" class="w-full h-full object-cover rounded-lg shadow-sm">
                    </div>
                    <div>
                        <h3 class="text-white font-black text-xs sm:text-sm tracking-widest uppercase italic">Funki Chat</h3>
                        <div class="flex items-center gap-1.5 text-green-400">
                            ...
                            <p class="text-[9px] sm:text-[10px] font-bold uppercase tracking-wider">Bereit</p>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex bg-slate-50 p-1.5 sm:p-2 gap-1.5 sm:gap-2 border-b border-slate-100 shrink-0">
                <button wire:click="setMode('chat')" class="flex-1 py-2 sm:py-2.5 rounded-xl sm:rounded-2xl text-[9px] sm:text-[10px] font-black uppercase tracking-wider transition-all {{ $activeMode === 'chat' ? 'bg-white shadow-sm text-blue-600 border border-blue-100' : 'text-slate-400 hover:bg-slate-100' }}">
                    Chat
                </button>
                @if(auth()->guard('admin')->check())
                    <button wire:click="setMode('logs')" class="flex-1 py-2 sm:py-2.5 rounded-xl sm:rounded-2xl text-[9px] sm:text-[10px] font-black uppercase tracking-wider transition-all {{ $activeMode === 'logs' ? 'bg-white shadow-sm text-blue-600 border border-blue-100' : 'text-slate-400 hover:bg-slate-100' }}">
                        System Log
                    </button>
                @endif
            </div>
            ...
            <div class="flex-1 overflow-hidden flex flex-col bg-white">
                @if($activeMode === 'chat')
                    <div class="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4 sm:space-y-5 bg-slate-50/20 custom-scrollbar">
                        @foreach($messages as $msg)
                            <div class="flex {{ $msg['role'] === 'user' ? 'justify-end' : 'justify-start' }}">
                                <div class="max-w-[85%] rounded-[1.5rem] px-4 py-2.5 sm:px-5 sm:py-3 text-xs sm:text-sm shadow-sm {{ $msg['role'] === 'user' ? 'bg-slate-900 text-white rounded-br-none' : 'bg-white text-slate-700 border border-slate-100 rounded-bl-none font-medium' }}">
                                    {!! nl2br(e($msg['content'])) !!}
                                </div>
                            </div>
                        @endforeach
                        @if($isTyping)
                            <div class="flex justify-start animate-pulse">
                                <div class="bg-slate-100 rounded-[1.5rem] px-4 py-2.5 text-slate-400 text-[10px] font-bold uppercase tracking-widest">Funki schreibt...</div>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="p-3 sm:p-4 bg-white border-t border-slate-50 shrink-0">
                        <form wire:submit.prevent="sendMessage" class="relative flex items-center bg-slate-100 rounded-[1.5rem] p-1.5 shadow-inner transition-all group focus-within:bg-white focus-within:ring-2 focus-within:ring-primary/10">
                            <input wire:model="input" type="text" placeholder="Frag Funki etwas..." class="flex-1 bg-transparent border-0 pl-4 py-2.5 sm:py-3 text-xs sm:text-sm focus:ring-0 outline-none">

                            <button type="submit" class="bg-primary text-white w-10 h-10 sm:w-11 sm:h-11 rounded-full shadow-lg hover:bg-primary-dark active:scale-95 transition-all flex items-center justify-center group/send shrink-0">
        ...
        {{-- TRIGGER BUTTON (Deutlich kleiner) --}}
        <button wire:click="toggleChat" class="pointer-events-auto relative group outline-none shrink-0 transition-all active:scale-90">
            ...
            {{-- Größe reduziert von w-20/h-20 auf w-14/h-14 (mobil) und w-16/h-16 (desktop) --}}
            <div class="w-14 h-14 sm:w-16 sm:h-16 bg-slate-900 rounded-[1.2rem] sm:rounded-[1.4rem] p-1 shadow-2xl transition-all duration-500 transform group-hover:scale-110 group-hover:-rotate-3 flex items-center justify-center border-2 border-white relative z-10 overflow-hidden shrink-0">
                <img src="{{ asset('images/projekt/funki/funki_selfie.png') }}" class="w-full h-full object-cover rounded-[0.9rem] sm:rounded-[1rem] shadow-inner">

                {{-- OS Badge (Ebenfalls verkleinert) --}}
                <div x-show="!$wire.isOpen" x-transition.opacity class="absolute top-1 right-1 flex h-4 w-4 sm:h-5 sm:w-5">
                    <span class="animate-ping absolute inset-0 rounded-full bg-blue-400 opacity-75"></span>
                    <span class="relative h-4 w-4 sm:h-5 sm:w-5 bg-blue-600 border sm:border-2 border-white rounded-full flex items-center justify-center text-[6px] sm:text-[8px] font-black text-white shadow-sm italic">
                        OS
                    </span>
                </div>
            </div>
