<div>
    <div class="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
        <div class="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mt-8">
            <div>
                <h2 class="text-3xl font-black text-white mb-2 uppercase tracking-wider font-mono">Interne Agenten</h2>
                <p class="text-gray-400 font-mono text-sm max-w-2xl">Verwalte hier das interne Multi-Agenten-System. Jeder Agent (Bot) hat eine eigene Identität und spezifische Fähigkeiten.</p>
            </div>
                    ...
                    <div class="relative z-10 flex items-start justify-between mb-5">
                        <div class="flex items-center gap-4">
                            @if($agent->profile_picture)
                                <div class="h-14 w-14 rounded-2xl overflow-hidden border border-{{ $agentColor }}/30 shadow-[0_0_15px_currentColor] text-{{ $agentColor }} bg-gray-900 group-hover:scale-110 transition-transform relative">
                            ...
                            @else
                                <div class="h-14 w-14 rounded-2xl flex items-center justify-center bg-{{ $agentColor }}/20 text-{{ $agentColor }} border border-{{ $agentColor }}/30 shadow-[0_0_15px_currentColor] group-hover:scale-110 transition-transform relative">
                                    <div class="absolute top-1 right-1 w-2.5 h-2.5 rounded-full {{ $statusColor }} border-2 border-gray-900 shadow-sm z-20"></div>
                    ...
                    <div class="relative z-10 pt-4 border-t border-gray-800/80 flex flex-col gap-2 text-[11px] font-mono uppercase tracking-widest">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="flex items-center gap-1.5 text-indigo-500/70 group-hover:text-indigo-400 transition-colors">
                                ...
                                LLM: {{ explode(' ', $agent->model ?? 'Standard')[0] }}
                            </span>
                        </div>
                        ...
                        <div class="mt-4 pt-4 border-t border-gray-800/80 flex items-center justify-between">
                            <button wire:click.stop="pingTest('{{ $agent->id }}')" class="px-3 py-1.5 bg-gray-900/50 hover:bg-primary/20 hover:text-primary text-gray-400 border border-gray-700 hover:border-primary/50 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all shadow-inner flex items-center gap-2 group/ping">
                                <span wire:loading.remove wire:target="pingTest('{{ $agent->id }}')" class="flex items-center gap-2"><x-heroicon-o-signal class="w-3.5 h-3.5 group-hover/ping:animate-pulse" /> Ping Test</span>
                                <span wire:loading wire:target="pingTest('{{ $agent->id }}')" class="flex items-center gap-2 text-primary opacity-80"><svg class="animate-spin h-3.5 w-3.5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg> Pinging...</span>
                            </button>
